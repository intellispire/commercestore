...
/**
 * Setup the global "cs_orders" database table
 *
 * @since 3.0
 */
final class Orders extends Table {
	...
	public function create() {
		...
		if ( ! empty( $created ) ) {
			...
			$last_payment_id = $this->get_db()->get_var( "SELECT ID FROM {$this->get_db()->prefix}posts WHERE post_type = 'cs_payment' ORDER BY ID DESC LIMIT 1;" );

			if ( ! empty( $last_payment_id ) ) {
				update_option( 'cs_v3_migration_pending', $last_payment_id, false );
