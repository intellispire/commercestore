...
class CS_Logging {
	...
	/**
	 * Set up the CommerceStore Logging Class
	 *
	 * @since 1.3.1
	 */
	...
	public function log_types() {
		return apply_filters( 'cs_log_types', array(
			'sale',
			'file_download',
			'gateway_error',
			'api_request'
	...
	*/
	...
	public function insert_log( $log_data = array(), $log_meta = array() ) {
		...
		if ( 'api_request' === $args['log_type'] ) {
			$insert_method = 'cs_add_api_request_log';
			...
			$insert_method = 'cs_add_file_download_log';
		...
		if ( $log_id && ! empty( $log_meta ) ) {
			...
			if ( 'cs_add_api_request_log' === $insert_method ) {
				$add_meta_function = 'cs_add_api_request_log_meta';
			} elseif ( 'cs_add_file_download_log' === $insert_method ) {
		 ...
		 * @param array $log_meta Log meta data.
		 */
		do_action( 'cs_post_insert_log', $log_id, $args, $log_meta );
	...
	public function update_log( $log_data = array(), $log_meta = array(), $log_id = 0 ) {
		...
		do_action( 'cs_pre_update_log', $log_data, $log_meta, $log_id );
		...
		// Used to dynamically dispatch the method call to insert() to the correct class.
		$update_method        = 'cs_update_log';
		$update_meta_function = 'cs_update_log_meta';
		...
		if ( 'api_request' === $data['type'] ) {
			$update_meta_function = 'cs_update_api_request_log_meta';
			$legacy               = array(
				'user'         => 'user_id',
				'key'          => 'api_key',
				'token'        => 'token',
			...
			$update_meta_function = 'cs_update_file_download_log_meta';
			$legacy               = array(
				'file_id'    => 'file_id',
		...
		if ( is_callable( $update_meta_function ) ) {
			if ( 'cs_update_log' === $update_method && ! empty( $log_meta ) ) {
				foreach ( (array) $log_meta as $key => $meta ) {
		...
		do_action( 'cs_post_update_log', $log_id, $log_data, $log_meta );
	}
	...
	public function delete_logs( $object_id = 0, $type = null, $meta_query = null ) {
		...
		$func = rtrim( "cs_delete_{$log_type}", 's' );
		if ( ! is_callable( $func ) ) {
			return;
		}
	...
	public function setup_log_file() {
		...
		$this->filename = wp_hash( home_url( '/' ) ) . '-cs-debug.log';
	...
	/**
	 * Return the location of the log file that CS_Logging will use.
	 *
	 * @since 2.9.1
	 *
	 * @return string
	...
	/**
	 * Registers the cs_log post type.
	 *
	 * @since 1.3.1
	 * @deprecated 3.0 Due to migration to custom tables.
	 */
	public function register_post_type() {
		_cs_deprecated_function( __FUNCTION__, '3.0.0' );
	}
 ...
 */
function cs_record_log( $title = '', $message = '', $parent = 0, $type = null ) {
	$cs_logs = CS()->debug_log;

	return $cs_logs->add( $title, $message, $parent, $type );
}
 ...
 */
function cs_debug_log( $message = '', $force = false ) {
	$cs_logs = CS()->debug_log;

	if ( cs_is_debug_mode() || $force ) {
 ...
 */
function cs_debug_log_exception( $exception ) {
