<?php
/**
 * License handler for CommerceStore
 *
 * This class should simplify the process of adding license information
 * to new CommerceStore extensions.
 *
 * @version 1.1
 */
...
class CS_License {
	...
	private $version;
	private $author;
	private $api_url = 'https://commercestore.com/cs-sl-api/';
	...
	public function __construct( $_file, $_item_name, $_version, $_author, $_optname = null, $_api_url = null, $_item_id = null ) {
		...
		$this->item_shortname = 'cs_' . preg_replace( '/[^a-zA-Z0-9_\s]/', '', str_replace( ' ', '_', strtolower( $this->item_name ) ) );
		$this->version        = $_version;
		$this->license        = trim( cs_get_option( $this->item_shortname . '_license_key', '' ) );
		$this->author         = $_author;
		 ...
		 */
		if ( ! empty( $_optname ) ) {
			$opt = cs_get_option( $_optname, false );
		 ...
		 * If we relied on the options table (`cs_%_license_active`) then we could accidentally
		 * be picking up plugins that have since been deactivated.
		 *
		 * @see \CS\Admin\Promos\Notices\License_Upgrade_Notice::__construct()
		 */
		if ( ! empty( $this->license ) ) {
			global $cs_licensed_products;
			if ( ! is_array( $cs_licensed_products ) ) {
				$cs_licensed_products = array();
			}
			$cs_licensed_products[] = $this->item_shortname;
		}
	...
	private function includes() {
		if ( ! class_exists( 'CS_SL_Plugin_Updater' ) )  {
			require_once 'CS_SL_Plugin_Updater.php';
		}
	}
	...
	private function hooks() {
		...
		// Register settings
		add_filter( 'cs_settings_licenses', array( $this, 'settings' ), 1 );

		// Display help text at the top of the Licenses tab
		add_action( 'cs_settings_tab_top', array( $this, 'license_help_text' ) );
		...
		if ( cs_doing_cron() ) {
			add_action( 'cs_weekly_scheduled_events', array( $this, 'weekly_license_check' ) );
		}
		...
		add_filter( 'cs_beta_enabled_extensions', array( $this, 'register_beta_support' ) );

		// Add the CommerceStore version to the API parameters.
		add_filter( 'cs_sl_plugin_updater_api_params', array( $this, 'filter_sl_api_params' ), 10, 3 );
	}
	...
	public function auto_updater() {
		...
		if ( empty( $license ) ) {
			$pass_manager = new \CS\Admin\Pass_Manager();
		...
		$args = array(
			...
			'license'   => $license,
			'author'    => $this->author,
			'beta'      => function_exists( 'cs_extension_has_beta_support' ) && cs_extension_has_beta_support( $this->item_shortname ),
		);
	...
	public function settings( $settings ) {
		return array_merge( $settings, array(
			array(
				...
				'name'    => sprintf( __( '%1$s', 'commercestore' ), $this->item_name ),
				'desc'    => '',
				'type'    => 'license_key',
	...
	public function license_help_text( $active_tab = '' ) {
		...
		echo '<p>' . sprintf(
			__( 'Enter your extension license keys here to receive updates for purchased extensions. If your license key has expired, please <a href="%s" target="_blank">renew your license</a>.', 'commercestore' ),
			'http://docs.commercestore.com/article/1000-license-renewal'
		) . '</p>';
	...
	/**
	 * If the supplied license key is for a pass, updates the `cs_pass_licenses` option with
	 * the pass ID and the date it was checked.
	 *
	 * Note: It's intentional that the `cs_pass_licenses` option is always updated, even if
	...
	private function maybe_set_pass_flag( $license, $api_data ) {
		$passes = get_option( 'cs_pass_licenses' );
	...
	public function activate_license() {
		...
		if ( empty( $_POST['cs_settings'][ $this->item_shortname . '_license_key'] ) ) {
		...
		$license = sanitize_text_field( $_POST['cs_settings'][ $this->item_shortname . '_license_key'] );
		...
		if ( ! empty( $api_data->license ) && 'valid' === $api_data->license ) {
			delete_option( 'cs_licensed_extensions' );
		}
	...
	public function deactivate_license() {
		...
		if ( ! isset( $_POST['cs_settings'][ $this->item_shortname . '_license_key'] ) ) {
			return;
		}
	...
	public function notices() {
		...
		if ( ! empty( $messages ) ) {
			foreach( $messages as $message ) {
				echo '<div class="cs-notice error">';
					echo '<p>' . $message . '</p>';
				echo '</div>';
			}
		}
	...
	/**
	 * Adds the CommerceStore version to the API parameters.
	 *
	 * @since 2.11
	...
	public function filter_sl_api_params( $api_params, $api_data, $plugin_file ) {
		...
		if ( $this->file === $plugin_file ) {
			$api_params['commercestore_version'] = defined( 'CS_VERSION' ) ? CS_VERSION : '';
		}
