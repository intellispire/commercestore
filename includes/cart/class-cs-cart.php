...
class CS_Cart {
	 ...
	 *
	 * @link https://github.com/commercestore/commercestore/issues/8455
	 *
	 * @since 3.0
	 * @return float
	 */
	...
	public function get_contents_from_session() {
		$cart = CS()->session->get( 'cs_cart' );
		$this->contents = $cart;

		do_action( 'cs_cart_contents_loaded_from_session', $this );
	}
	...
	public function get_discounts_from_session() {
		$discounts = CS()->session->get( 'cart_discounts' );
		$this->discounts = $discounts;

		do_action( 'cs_cart_discounts_loaded_from_session', $this );
	}
	...
	public function get_contents() {
		if ( ! did_action( 'cs_cart_contents_loaded_from_session' ) ) {
			$this->get_contents_from_session();
		}
		...
		$this->contents = apply_filters( 'cs_cart_contents', $cart );

		do_action( 'cs_cart_contents_loaded' );
	...
	public function get_contents_details() {
		global $cs_is_last_cart_item, $cs_flat_discount_total;
		...
		foreach ( $this->contents as $key => $item ) {
			if( $key >= $length ) {
				$cs_is_last_cart_item = true;
			}

			$item['quantity'] = cs_item_quantities_enabled() ? absint( $item['quantity'] ) : 1;
			...
			$discount   = apply_filters( 'cs_get_cart_content_details_item_discount_amount', $discount, $item );
			...
			$details[ $key ]  = array(
				...
				'quantity'    => $quantity,
				'discount'    => round( $discount, cs_currency_decimal_filter() ),
			...
			if ( $cs_is_last_cart_item ) {
				$cs_is_last_cart_item   = false;
				$cs_flat_discount_total = 0.00;
			}
		}
	...
	public function update_cart() {
		CS()->session->set( 'cs_cart', $this->contents );
	}
	...
	public function has_discounts() {
		...
		$this->has_discounts = apply_filters( 'cs_cart_has_discounts', $has_discounts );
	...
	public function get_quantity() {
		...
		$this->quantity = apply_filters( 'cs_get_cart_quantity', $total_quantity, $this->contents );
	 ...
	 * Add to cart
	 *
	 * As of CommerceStore 2.7, items can only be added to the cart when the object passed extends CS_Cart_Item
	 *
	 * @since 2.7
	...
	public function add( $download_id, $options = array() ) {
		...
		do_action( 'cs_pre_add_to_cart', $download_id, $options );
		 ...
		 */
		$this->contents = apply_filters( 'cs_pre_add_to_cart_contents', $this->contents, $download_id, $options );

		$quantities_enabled = cs_item_quantities_enabled() && ! cs_download_quantities_disabled( $download_id );
		...
		if ( $download->has_variable_prices()  && ! isset( $options['price_id'] ) ) {
			...
			$options['price_id'] = get_post_meta( $download->ID, '_cs_default_price_id', true );
		}
		...
		foreach ( $items as &$item ) {
			$item = apply_filters( 'cs_add_to_cart_item', $item );
			$to_add = $item;
			...
			if ( cs_item_in_cart( $to_add['id'], $to_add['options'] ) && cs_item_quantities_enabled() ) {
				$key = cs_get_item_position_in_cart( $to_add['id'], $to_add['options'] );
		...
		do_action( 'cs_post_add_to_cart', $download_id, $options, $items );
	...
	public function remove( $key ) {
		...
		do_action( 'cs_pre_remove_from_cart', $key );
		...
		do_action( 'cs_post_remove_from_cart', $key, $item_id );
	...
	public function remove_item_url( $cart_key ) {
		...
		$current_page = cs_doing_ajax()
			? cs_get_checkout_uri()
			: cs_get_current_page_url();

		$remove_url = cs_add_cache_busting( add_query_arg( array(
			'cart_item'  => $cart_key,
			'cs_action' => 'remove'
		), $current_page ) );

		return apply_filters( 'cs_remove_item_url', $remove_url );
	}
	...
	public function remove_fee_url( $fee_id = '' ) {
		...
		$remove_url = add_query_arg( array(
			'fee'        => $fee_id,
			'cs_action' => 'remove_fee',
			'nocache'    => 'true'
		), $current_page );

		return apply_filters( 'cs_remove_fee_url', $remove_url );
	}
	...
	public function empty_cart() {
		...
		CS()->session->set( 'cs_resume_payment', NULL );

		// Remove any active discounts
		$this->remove_all_discounts();
		$this->contents = array();

		do_action( 'cs_empty_cart' );
	}
	...
	public function remove_discount( $code = '' ) {
		...
		if ( $this->discounts ) {
			...
			CS()->session->set( 'cart_discounts', $this->discounts );
		}

		do_action( 'cs_cart_discount_removed', $code, $this->discounts );
		do_action( 'cs_cart_discounts_updated', $this->discounts );
	...
	public function remove_all_discounts() {
		...
		do_action( 'cs_cart_discounts_removed' );
	}
	 ...
	 *
	 * @since 2.7
	 * @since 3.0 Use `cs_get_item_discount_amount()` for calculations.
	 *
	...
	public function get_item_discount_amount( $item = array(), $discount = false ) {
		...
		if ( ! isset( $item['options'] ) ) {
			...
			/*
			 * Support for variable pricing when calling `cs_get_cart_item_discount_amount()`
		...
		$discount_amount = cs_get_item_discount_amount( $item, $this->get_contents(), $discounts );
		...
		$discounted_amount = apply_filters(
			'cs_get_cart_item_discounted_amount',
			$discounted_amount,
			$discounts,
			$item,
			$item_price
		...
		$discount_amount = round( ( $item_price - $discounted_amount ), cs_currency_decimal_filter() );
	...
	public function display_cart_discount( $echo = false ) {
		...
		$discount_id  = cs_get_discount_id_by_code( $discounts[0] );
		$amount       = cs_format_discount_rate( cs_get_discount_type( $discount_id ), cs_get_discount_amount( $discount_id ) );
	...
	public function is_item_in_cart( $download_id = 0, $options = array() ) {
		...
		return (bool) apply_filters( 'cs_item_in_cart', $ret, $download_id, $options );
	}
	...
	public function get_item_quantity( $download_id = 0, $options = array() ) {
		...
		$quantity = isset( $this->contents[ $key ]['quantity'] ) && cs_item_quantities_enabled() ? $this->contents[ $key ]['quantity'] : 1;

		if ( $quantity < 1 ) {
			$quantity = 1;
		}

		return absint( apply_filters( 'cs_get_cart_item_quantity', $quantity, $download_id, $options ) );
	}
	...
	public function set_item_quantity( $download_id = 0, $quantity = 1, $options = array() ) {
		...
		do_action( 'cs_after_set_cart_item_quantity', $download_id, $quantity, $options, $this->contents );
	...
	public function item_price( $item_id = 0, $options = array() ) {
		...
		if ( ! cs_is_free_download( $item_id, $price_id ) && ! cs_download_is_tax_exclusive( $item_id ) ) {
			if ( cs_prices_show_tax_on_checkout() && ! cs_prices_include_tax() ) {
				$price += cs_get_cart_item_tax( $item_id, $options, $price );
			}
			...
			if ( cs_display_tax_rate() ) {
				$label = '&nbsp;&ndash;&nbsp;';

				if ( cs_prices_show_tax_on_checkout() ) {
					$label .= sprintf( __( 'includes %s tax', 'commercestore' ), cs_get_formatted_tax_rate() );
				} else {
					$label .= sprintf( __( 'excludes %s tax', 'commercestore' ), cs_get_formatted_tax_rate() );
				}

				$label = apply_filters( 'cs_cart_item_tax_description', $label, $item_id, $options );
			}
		}

		$price = cs_currency_filter( cs_format_amount( $price ) );

		return apply_filters( 'cs_cart_item_price_label', $price . $label, $item_id, $options );
	}
	...
	public function get_item_price( $download_id = 0, $options = array(), $remove_tax_from_inclusive = false ) {
		$price = 0;
		$variable_prices = cs_has_variable_prices( $download_id );
		...
		if ( $remove_tax_from_inclusive && cs_prices_include_tax() ) {
			$price -= $this->get_item_tax( $download_id, $options, $price );
		}

		return apply_filters( 'cs_cart_item_price', $price, $download_id, $options );
	}
	...
	public function get_item_final_price( $item_key = 0 ) {
		...
		return apply_filters( 'cs_cart_item_final_price', $final_price, $item_key );
	}
	...
	public function get_item_tax( $download_id = 0, $options = array(), $subtotal = '' ) {
		...
		if ( ! cs_download_is_tax_exclusive( $download_id ) ) {
			$country = ! empty( $_POST['billing_country'] ) ? $_POST['billing_country'] : false;
			$state   = ! empty( $_POST['card_state'] )      ? $_POST['card_state']      : false;

			$tax = cs_calculate_tax( $subtotal, $country, $state, true, $this->get_tax_rate() );
		}

		$tax = max( $tax, 0 );

		return apply_filters( 'cs_get_cart_item_tax', $tax, $download_id, $options, $subtotal );
	}
	...
	public function get_fees( $type = 'all', $download_id = 0, $price_id = null ) {
		return CS()->fees->get_fees( $type, $download_id, $price_id );
	}
	...
	public function get_all_fees() {
		$this->fees = CS()->fees->get_fees( 'all' );
		return $this->fees;
	}
	...
	public function get_items_subtotal( $items ) {
		...
		$this->subtotal = apply_filters( 'cs_get_cart_items_subtotal', $subtotal );
	...
	public function get_discountable_subtotal( $code_id ) {
		...
		$excluded_products = cs_get_discount_excluded_products( $code_id );
		...
		return apply_filters( 'cs_get_cart_discountable_subtotal', $subtotal );
	}
	...
	public function get_discounted_amount( $discounts = false ) {
		...
		return apply_filters( 'cs_get_cart_discounted_amount', $amount );
	}
	...
	public function get_subtotal() {
		...
		return apply_filters( 'cs_get_cart_subtotal', $subtotal );
	}
	...
	public function subtotal() {
		return esc_html( cs_currency_filter( cs_format_amount( cs_get_cart_subtotal() ) ) );
	}
	...
	public function get_total( $discounts = false ) {
		...
		$this->total = (float) apply_filters( 'cs_get_cart_total', $total );
	...
	public function total( $echo ) {
		$total = apply_filters( 'cs_cart_total', cs_currency_filter( cs_format_amount( $this->get_total() ) ) );
	...
	public function get_total_fees() {
		...
		return apply_filters( 'cs_get_fee_total', $fee_total, $this->fees );
	}
	...
	public function get_item_price_name( $item = array() ) {
		...
		$prices   = cs_get_variable_prices( $item['id'] );
		$name     = ! empty( $prices[ $price_id ] ) ? $prices[ $price_id ]['name'] : '';

		return apply_filters( 'cs_get_cart_item_price_name', $name, $item['id'], $price_id, $item );
	}
	...
	public function get_item_name( $item = array() ) {
		...
		if ( cs_has_variable_prices( $item['id'] ) && false !== cs_get_cart_item_price_id( $item ) ) {
			$item_title .= ' - ' . cs_get_cart_item_price_name( $item );
		}

		return apply_filters( 'cs_get_cart_item_name', $item_title, $item['id'], $item );
	}
	...
	public function get_tax() {
		...
		$cart_tax = apply_filters( 'cs_get_cart_tax', cs_sanitize_amount( $cart_tax ) );
	...
	public function tax( $echo = false ) {
		...
		$cart_tax = cs_currency_filter( cs_format_amount( $cart_tax ) );

		$tax = max( $cart_tax, 0 );
		$tax = apply_filters( 'cs_cart_tax', $cart_tax );
	...
	public function get_tax_on_fees() {
		...
		if ( $fees ) {
			foreach ( $fees as $fee_id => $fee ) {
				...
				$tax += cs_calculate_tax( $fee['amount'], '', '', true, $this->get_tax_rate() );
				remove_filter( 'cs_prices_include_tax', '__return_false' );
			}
		}

		return apply_filters( 'cs_get_cart_fee_tax', $tax );
	}
	...
	public function is_saving_enabled() {
		return cs_get_option( 'enable_cart_saving', false );
	}
	...
	public function is_saved() {
		...
		$saved_cart = get_user_meta( get_current_user_id(), 'cs_saved_cart', true );
		...
		if ( is_user_logged_in() ) {
			...
			if ( $saved_cart === CS()->session->get( 'cs_cart' ) ) {
				return false;
			}

			return true;
		} else {
			if ( ! isset( $_COOKIE['cs_saved_cart'] ) ) {
				return false;
			}

			if ( json_decode( stripslashes( $_COOKIE['cs_saved_cart'] ), true ) === CS()->session->get( 'cs_cart' ) ) {
				return false;
			}
	...
	public function save() {
		...
		// Get cart & cart token
		$cart  = CS()->session->get( 'cs_cart' );
		$token = cs_generate_cart_token();

		if ( is_user_logged_in() ) {
			$user_id = get_current_user_id();
			update_user_meta( $user_id, 'cs_saved_cart', $cart,  false );
			update_user_meta( $user_id, 'cs_cart_token', $token, false );
		} else {
			$cart    = json_encode( $cart );
			$expires = time() + WEEK_IN_SECONDS;
			@setcookie( 'cs_saved_cart', $cart,  $expires, COOKIEPATH, COOKIE_DOMAIN );
			@setcookie( 'cs_cart_token', $token, $expires, COOKIEPATH, COOKIE_DOMAIN );
		}
		...
		$messages['cs_cart_save_successful'] = sprintf(
			'<strong>%1$s</strong>: %2$s',
			__( 'Success', 'commercestore' ),
			__( 'Cart saved successfully. You can restore your cart using this URL:', 'commercestore' ) . ' ' . '<a href="' .  cs_get_checkout_uri() . '?cs_action=restore_cart&cs_cart_token=' . $token . '">' .  cs_get_checkout_uri() . '?cs_action=restore_cart&cs_cart_token=' . $token . '</a>'
		);

		// Set these messages in the session
		CS()->session->set( 'cs_cart_messages', $messages );
	...
	public function restore() {
		...
		$user_id    = get_current_user_id();
		$saved_cart = get_user_meta( $user_id, 'cs_saved_cart', true );
		...
		if ( is_user_logged_in() && $saved_cart ) {
			...
			if ( isset( $_GET['cs_cart_token'] ) && ! hash_equals( $_GET['cs_cart_token'], $token ) ) {
				$messages['cs_cart_restoration_failed'] = sprintf( '<strong>%1$s</strong>: %2$s', __( 'Error', 'commercestore' ), __( 'Cart restoration failed. Invalid token.', 'commercestore' ) );
				CS()->session->set( 'cs_cart_messages', $messages );
			}

			delete_user_meta( $user_id, 'cs_saved_cart' );
			delete_user_meta( $user_id, 'cs_cart_token' );

			if ( isset( $_GET['cs_cart_token'] ) && $_GET['cs_cart_token'] != $token ) {
				return new WP_Error( 'invalid_cart_token', __( 'The cart cannot be restored. Invalid token.', 'commercestore' ) );
			}
		} elseif ( ! is_user_logged_in() && isset( $_COOKIE['cs_saved_cart'] ) && $token ) {
			$saved_cart = $_COOKIE['cs_saved_cart'];

			if ( ! hash_equals( $_GET['cs_cart_token'], $token ) ) {
				$messages['cs_cart_restoration_failed'] = sprintf( '<strong>%1$s</strong>: %2$s', __( 'Error', 'commercestore' ), __( 'Cart restoration failed. Invalid token.', 'commercestore' ) );
				CS()->session->set( 'cs_cart_messages', $messages );
			...
			setcookie( 'cs_saved_cart', '', time()-3600, COOKIEPATH, COOKIE_DOMAIN );
			setcookie( 'cs_cart_token', '', time()-3600, COOKIEPATH, COOKIE_DOMAIN );
		}

		$messages['cs_cart_restoration_successful'] = sprintf( '<strong>%1$s</strong>: %2$s', __( 'Success', 'commercestore' ), __( 'Cart restored successfully.', 'commercestore' ) );
		CS()->session->set( 'cs_cart', $saved_cart );
		CS()->session->set( 'cs_cart_messages', $messages );
	...
	public function get_token() {
		...
		if ( is_user_logged_in() ) {
			$token = get_user_meta( $user_id, 'cs_cart_token', true );
		} else {
			$token = isset( $_COOKIE['cs_cart_token'] ) ? $_COOKIE['cs_cart_token'] : false;
		}

		return apply_filters( 'cs_get_cart_token', $token, $user_id );
	}
	...
	public function generate_token() {
		return apply_filters( 'cs_generate_cart_token', md5( mt_rand() . time() ) );
	}
}
