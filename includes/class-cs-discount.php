 ...
 *
 * @since 2.7
 * @since 3.0 Extends CS\Database\Rows\Adjustment instead of CS_DB_Discount
 *
 * @property int $id
 ...
 */
class CS_Discount extends Adjustment {
	...
	public function __construct( $_id_or_code_or_name = false, $by_code = false, $by_name = false ) {
		...
		// Default to ID
			...
			$discount = cs_get_discount( absint( $_id_or_code_or_name ) );
		}
	...
	public function __get( $key = '' ) {
		...
		// Other...
			...
			return new WP_Error( 'cs-discount-invalid-property', sprintf( __( 'Can\'t get property %s', 'commercestore' ), $key ) );
		}
	}
	...
	/**
	 * Converts the instance of the CS_Discount object into an array for special cases.
	 *
	 * @since 2.7
	 *
	 * @return array CS_Discount object as an array.
	 */
	...
	private function find_by_name( $name = '' ) {
		return cs_get_discount_by( 'name', $name );
	}
	...
	private function setup_discount( $discount = null ) {
		...
		/**
		 * Fires before the instance of the CS_Discount object is set up.
		 *
		 * @since 2.7
		 *
		 * @param object CS_Discount      CS_Discount instance of the discount object.
		 * @param object WP_Post $discount WP_Post instance of the discount object.
		 */
		do_action( 'cs_pre_setup_discount', $this, $discount );
		 ...
		 * pulled in from the `cs_adjustmentmeta` table.
		 */
		$this->excluded_products = (array) cs_get_adjustment_meta( $this->id, 'excluded_product',    false );
		$this->product_reqs      = (array) cs_get_adjustment_meta( $this->id, 'product_requirement', false );
		$this->product_condition = (string) cs_get_adjustment_meta( $this->id, 'product_condition', true );

		/**
		 * Fires after the instance of the CS_Discount object is set up. Allows extensions to add items to this object via hook.
		 *
		 * @since 2.7
		 *
		 * @param object CS_Discount      CS_Discount instance of the discount object.
		 * @param object WP_Post $discount WP_Post instance of the discount object.
		 */
		do_action( 'cs_setup_discount', $this, $discount );
	...
	public function get_meta( $key = '', $single = true ) {
		return cs_get_adjustment_meta( $this->id, $key, $single );
	}
	...
	public function update_meta( $key, $value = '', $prev_value = '' ) {
		$filter_key = '_cs_discount_' . $key;

		/**
		 * Filters the meta value being updated.
		 * The key is prefixed with `_cs_discount_` for 2.9 backwards compatibility.
		 *
		 * @param mixed $value Value being set.
		 * @param int   $id    Discount ID.
		 */
		$value = apply_filters( 'cs_update_discount_meta_' . $filter_key, $value, $this->id );

		return cs_update_adjustment_meta( $this->id, $key, $value, $prev_value );
	}
	...
	public function get_code() {
		 ...
		 * @param int    $ID   Discount ID.
		 */
		return apply_filters( 'cs_get_discount_code', $this->code, $this->id );
	}
	...
	public function get_status() {
		 ...
		 * @param int    $ID   Discount ID.
		 */
		return apply_filters( 'cs_get_discount_status', $this->status, $this->id );
	}
	...
	public function get_status_label() {
		...
		switch ( $this->status ) {
			case '':
				$label = __( 'None',     'commercestore' );
				break;
			case 'draft':
				$label = __( 'Draft',    'commercestore' );
				break;
			case 'expired':
				$label = __( 'Expired',  'commercestore' );
				break;
			case 'inactive':
				$label = __( 'Inactive', 'commercestore' );
				break;
			case 'active':
				$label = __( 'Active',   'commercestore' );
				break;
			case 'inherit':
				if ( ! empty( $this->parent ) ) {
					$parent = cs_get_discount( $this->parent );
		 ...
		 * @param int    $id     Discount ID.
		 */
		return apply_filters( 'cs_get_discount_status_label', $label, $this->status, $this->id );
	}
	...
	public function get_type() {
		 ...
		 * @param int    $ID   Discount ID.
		 */
		return apply_filters( 'cs_get_discount_type', $this->amount_type, $this->id );
	}
	...
	public function get_amount() {
		 ...
		 * @param int    $ID    Discount ID.
		 */
		return (float) apply_filters( 'cs_get_discount_amount', $this->amount, $this->id );
	}
	...
	public function get_product_reqs() {
		 ...
		 * @param int   $ID           Discount ID.
		 */
		return (array) apply_filters( 'cs_get_discount_product_reqs', $this->product_reqs, $this->id );
	}
	...
	public function get_scope() {
		$legacy_value = apply_filters( 'cs_discount_is_not_global', null, $this->id );

		if ( ! is_null( $legacy_value ) ) {
			$this->scope = $legacy_value ? 'global' : 'not_global';
		}

		return apply_filters( 'cs_get_discount_scope', $this->scope, $this->id );
	}
	...
	public function get_product_condition() {
		 ...
		 * @param int    $ID                Discount ID.
		 */
		return apply_filters( 'cs_discount_product_condition', $this->product_condition, $this->id );
	}
	...
	public function get_excluded_products() {
		 ...
		 * @param int   $ID                Discount ID.
		 */
		return (array) apply_filters( 'cs_get_discount_excluded_products', $this->excluded_products, $this->id );
	}
	...
	public function get_start_date() {
		 ...
		 * @param int    $ID    Discount ID.
		 */
		return apply_filters( 'cs_get_discount_start', $this->start_date, $this->id );
	}
	...
	public function get_expiration() {
		 ...
		 * @param int   $ID          Discount ID.
		 */
		return apply_filters( 'cs_get_discount_expiration', $this->end_date, $this->id );
	}
	...
	public function get_uses() {
		 ...
		 * @param int $ID       Discount ID.
		 */
		return (int) apply_filters( 'cs_get_discount_uses', $this->use_count, $this->id );
	}
	...
	public function get_max_uses() {
		 ...
		 * @param int $ID       Discount ID.
		 */
		return (int) apply_filters( 'cs_get_discount_max_uses', $this->max_uses, $this->id );
	}
	...
	public function get_min_price() {
		 ...
		 * @param int   $ID        Discount ID.
		 */
		return (float) apply_filters( 'cs_get_discount_min_price', $this->min_charge_amount, $this->id );
	}
	...
	public function get_once_per_customer() {
		 ...
		 * @param int  $ID            Discount ID.
		 */
		return (bool) apply_filters( 'cs_is_discount_single_use', $this->once_per_customer, $this->id );
	}
	...
	public function add( $args = array() ) {
		...
		if ( ! empty( $this->id ) && $this->exists() ) {
			 ...
			 */
			$args = apply_filters( 'cs_insert_discount', $args );
			 ...
			 * @param array $args Discount args.
			 */
			$args = apply_filters( 'cs_insert_discount_args', $args, $args );
			 ...
			 * @param array $args Discount args.
			 */
			do_action( 'cs_pre_insert_discount', $args );

			foreach ( $args as $key => $value ) {
				$this->{$key} = $value;
			}

			// We have to ensure an ID is not passed to cs_add_discount()
			unset( $args['id'] );

			$id = cs_add_discount( $args );
				...
				if ( isset( $args['excluded_products'] ) ) {
					if ( is_array( $args['excluded_products'] ) ) {
						foreach ( $args['excluded_products'] as $product ) {
							cs_add_adjustment_meta( $this->id, 'excluded_product', absint( $product ) );
						}
					}
				}
				...
				if ( isset( $args['product_reqs'] ) ) {
					if ( is_array( $args['product_reqs'] ) ) {
						foreach ( $args['product_reqs'] as $product ) {
							cs_add_adjustment_meta( $this->id, 'product_requirement', absint( $product ) );
						}
					}
				}
			}
			 ...
			 */
			do_action( 'cs_post_insert_discount', $args, $this->id );
	...
	public function update( $args = array() ) {
		 ...
		 * @param int   $ID   Discount ID.
		 */
		$args = apply_filters( 'cs_update_discount', $args, $this->id );
		$args = $this->sanitize_columns( $args );
		...
		if ( isset( $args['excluded_products'] ) ) {
			// Reset meta
			cs_delete_adjustment_meta( $this->id, 'excluded_product' );
		...
		if ( isset( $args['product_reqs'] ) ) {
			// Reset meta
			cs_delete_adjustment_meta( $this->id, 'product_requirement' );
		 ...
		 * @param int   $ID   Discount ID.
		 */
		do_action( 'cs_pre_update_discount', $args, $this->id );

		// If we are using the discounts DB
		if ( cs_update_discount( $this->id, $args ) ) {
			$discount = cs_get_discount( $this->id );
			$this->setup_discount( $discount );
			$ret = true;
		}
		 ...
		 * @param int   $ID   Discount ID.
		 */
		do_action( 'cs_post_update_discount', $args, $this->id );
	...
	public function update_status( $new_status = 'active' ) {
		 ...
		 */
		do_action( 'cs_pre_update_discount_status', $this->id, $new_status, $this->status );
		 ...
		 */
		do_action( 'cs_post_update_discount_status', $this->id, $new_status, $this->status );
	...
	public function is_started( $set_error = true ) {
		...
		if ( $this->start_date ) {
			...
			if ( $start_date < time() ) {
				...
				cs_set_error( 'cs-discount-error', _x( 'This discount is invalid.', 'error shown when attempting to use a discount before its start date', 'commercestore' ) );
			}
		} else {
		 ...
		 * @param bool $set_error Whether an error message be set in session.
		 */
		return apply_filters( 'cs_is_discount_started', $return, $this->id, $set_error );
	}
	...
	public function is_expired( $update = true ) {
		 ...
		 * @param int  $ID     Discount ID.
		 */
		return apply_filters( 'cs_is_discount_expired', $return, $this->id );
	}
	...
	public function is_maxed_out( $set_error = true ) {
		...
		if ( $this->uses >= $this->max_uses && ! empty( $this->max_uses ) ) {
			if ( $set_error ) {
				cs_set_error( 'cs-discount-error', __( 'This discount has reached its maximum usage.', 'commercestore' ) );
			}
		 ...
		 * @param bool $set_error Whether an error message be set in session.
		 */
		return apply_filters( 'cs_is_discount_maxed_out', $return, $this->id, $set_error );
	}
	...
	public function is_min_price_met( $set_error = true ) {
		...
		$cart_amount = cs_get_cart_discountable_subtotal( $this->id );

		if ( (float) $cart_amount >= (float) $this->min_charge_amount ) {
			$return = true;
		} elseif ( $set_error ) {
			cs_set_error( 'cs-discount-error', sprintf( __( 'Minimum order of %s not met.', 'commercestore' ), cs_currency_filter( cs_format_amount( $this->min_charge_amount ) ) ) );
		}
		 ...
		 * @param bool $set_error Whether an error message be set in session.
		 */
		return apply_filters( 'cs_is_discount_min_met', $return, $this->id, $set_error );
	}
	...
	public function is_used( $user = '', $set_error = true ) {
		...
		if ( $this->is_single_use ) {
			...
			if ( cs_get_component_interface( 'customer', 'table' )->exists() ) {
				$by_user_id = ! is_email( $user );

				$customer = new CS_Customer( $user, $by_user_id );
			...
			if ( $payments ) {
				foreach ( $payments as $payment ) {
					...
					if ( is_array( $discounts ) ) {
						...
						if ( false !== $key ) {
							if ( $set_error ) {
								cs_set_error( 'cs-discount-error', __( 'This discount has already been redeemed.', 'commercestore' ) );
							}
		 ...
		 * @param bool $set_error Whether an error message be set in session.
		 */
		return apply_filters( 'cs_is_discount_used', $return, $this->id, $user, $set_error );
	}
	...
	public function is_valid( $user = '', $set_error = true ) {
		...
		if ( cs_get_cart_contents() && $this->id ) {
			if (
		 ...
		 * @param bool $set_error Whether an error message be set in session.
		 */
		return apply_filters( 'cs_is_discount_valid', $return, $this->id, $this->code, $user, $set_error );
	}
	...
	public function is_active( $update = true, $set_error = true ) {
		...
		if ( $this->exists() ) {
			...
			if ( $this->is_expired( $update ) ) {
				if ( cs_doing_ajax() && $set_error ) {
					cs_set_error( 'cs-discount-error', __( 'This discount is expired.', 'commercestore' ) );
				}
			} elseif ( 'active' === $this->status ) {
				$return = true;
			} elseif ( cs_doing_ajax() && $set_error ) {
				cs_set_error( 'cs-discount-error', __( 'This discount is not active.', 'commercestore' ) );
			}
		}
		 ...
		 * @param bool $set_error Whether an error message be set in session.
		 */
		return apply_filters( 'cs_is_discount_active', $return, $this->id, $set_error );
	}
	...
	public function get_discounted_amount( $base_price ) {
		 ...
		 * @param CS_Discount $this Discount object.
		 */
		return apply_filters( 'cs_discounted_amount', $amount, $this );
	}
	...
	public function increase_usage() {
		 ...
		 * @param string $code      Discount code.
		 */
		do_action( 'cs_discount_increase_use_count', $this->use_count, $this->id, $this->code );
	...
	public function decrease_usage() {
		 ...
		 * @param string $code      Discount code.
		 */
		do_action( 'cs_discount_decrease_use_count', $this->use_count, $this->id, $this->code );
	...
	public static function convert_legacy_args( $args = array() ) {
		...
		// See https://github.com/commercestore/commercestore/issues/6806
		if ( ! isset( $args['status'] ) ) {
			$args['status'] = 'active';
		}
