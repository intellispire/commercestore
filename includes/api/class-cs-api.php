 ...
 * systems, such as mobile. This class is also used in the CommerceStore iOS App.
 *
 * @package     CS
...
class CS_API {
	...
	/**
	 * Instance of CommerceStore Stats class
	 *
	 * @var object
	 * @access private
	 * @since  1.7
	...
	public function __construct() {
		...
		foreach ( $this->get_versions() as $version => $class ) {
			require_once CS_PLUGIN_DIR . 'includes/api/class-cs-api-' . $version . '.php';
		}

		add_action( 'init', array( $this, 'add_endpoint' ) );
		add_action( 'wp', array( $this, 'process_query' ), - 1 );
		add_filter( 'query_vars', array( $this, 'query_vars' ) );
		add_action( 'cs_process_api_key', array( $this, 'process_api_key' ) );
		...
		// Setup CS_Stats instance
		$this->stats = new CS_Payment_Stats;
	...
	public function get_default_version() {
		...
		$version = get_option( 'cs_default_api_version' );

		if ( defined( 'CS_API_VERSION' ) ) {
			$version = CS_API_VERSION;
		} elseif ( ! $version ) {
			$version = 'v1';
		}
	...
	private function set_queried_version() {
		...
		$version = $wp_query->query_vars['cs-api'];
		...
		if ( strpos( $version, '/' ) ) {
			...
			$wp_query->query_vars['cs-api'] = str_replace( $version . '/', '', $wp_query->query_vars['cs-api'] );
	 ...
	 * @uses   CS_API::get_user()
	 * @uses   CS_API::invalid_key()
	 * @uses   CS_API::invalid_auth()
	 * @since  1.5
	 * @return bool
	 */
	private function validate_request() {
		...
		if ( ! empty( $wp_query->query_vars['cs-api'] ) && ( ! $this->is_public_query() || ! empty( $wp_query->query_vars['token'] ) ) ) {
		...
		} elseif ( ! empty( $wp_query->query_vars['cs-api'] ) && $this->is_public_query() ) {
			$this->is_valid_request = true;
	...
	private function is_public_query() {
		...
		$public_modes = apply_filters( 'cs_api_public_query_modes', array(
			'products',
		) );

		return in_array( $wp_query->query_vars['cs-api'], $public_modes );
	}
	...
	public function get_user( $key = '' ) {
		...
		$user = get_transient( md5( 'cs_api_user_' . $key ) );

		if ( false === $user ) {
			$user = $wpdb->get_var( $wpdb->prepare( "SELECT user_id FROM $wpdb->usermeta WHERE meta_key = %s LIMIT 1", $key ) );
			set_transient( md5( 'cs_api_user_' . $key ), $user, DAY_IN_SECONDS );
		}
	...
	public function get_user_public_key( $user_id = 0 ) {
		...
		$cache_key       = md5( 'cs_api_user_public_key' . $user_id );
		$user_public_key = get_transient( $cache_key );

		if ( empty( $user_public_key ) ) {
			$user_public_key = $wpdb->get_var( $wpdb->prepare( "SELECT meta_key FROM $wpdb->usermeta WHERE meta_value = 'cs_user_public_key' AND user_id = %d", $user_id ) );
	...
	public function get_user_secret_key( $user_id = 0 ) {
		...
		$cache_key       = md5( 'cs_api_user_secret_key' . $user_id );
		$user_secret_key = get_transient( $cache_key );

		if ( empty( $user_secret_key ) ) {
			$user_secret_key = $wpdb->get_var( $wpdb->prepare( "SELECT meta_key FROM $wpdb->usermeta WHERE meta_value = 'cs_user_secret_key' AND user_id = %d", $user_id ) );
	...
	private function missing_auth() {
		$error          = array();
		$error['error'] = __( 'You must specify both a token and API key!', 'commercestore' );
	...
	private function invalid_auth() {
		$error          = array();
		$error['error'] = __( 'Your request could not be authenticated!', 'commercestore' );
	...
	private function invalid_key() {
		$error          = array();
		$error['error'] = __( 'Invalid API key!', 'commercestore' );
	...
	private function invalid_version() {
		$error          = array();
		$error['error'] = __( 'Invalid API version!', 'commercestore' );
	...
	public function process_query() {
		...
		// Check for cs-api var. Get out if not present
		if ( empty( $wp_query->query_vars['cs-api'] ) ) {
			return;
		}
		...
		if ( ! defined( 'CS_DOING_API' ) ) {
			define( 'CS_DOING_API', true );
		}
		...
		$this->data = apply_filters( 'cs_api_output_data', $data, $this->endpoint, $this );
	...
	public function set_query_mode() {
		...
		$query = isset( $wp_query->query_vars['cs-api'] ) ? $wp_query->query_vars['cs-api'] : null;
		...
		if ( ! in_array( $query, $accepted ) ) {
			$error['error'] = __( 'Invalid query!', 'commercestore' );
	...
	public function per_page() {
		...
		return apply_filters( 'cs_api_results_per_page', $per_page );
	}
	...
	public function get_dates( $args = array() ) {
		...
		return apply_filters( 'cs_api_stat_dates', $dates );
	}
	...
	public function get_customers( $customer = null ) {
		...
		$customer_query = cs_get_customers( $args );
		$customer_count = 0;
		...
		if ( $customer_query ) {
			...
			foreach ( $customer_query as $customer_obj ) {
				...
				$customers['customers'][ $customer_count ]['stats']['total_downloads'] = cs_count_file_downloads_of_customer( $customer_obj->id );
			...
			$error['error'] = sprintf( __( 'Customer %s not found!', 'commercestore' ), $customer );

			return $error;

		} else {

			$error['error'] = __( 'No customers found!', 'commercestore' );

			return $error;

		}

		return apply_filters( 'cs_api_customers', $customers, $this );
	}
	...
	public function get_products( $args = array() ) {
		...
		if ( empty( $args['product'] ) ) {
			...
			if ( get_post_type( $args['product'] ) == CS_POST_TYPE ) {
				...
				$error['error'] = sprintf( __( 'Product %s not found!', 'commercestore' ), $args['product'] );

				return $error;
			}
		}

		return apply_filters( 'cs_api_products', $products, $this );
	}
	...
	public function get_product_data( $product_info ) {
		...
		$product['info']['tags']          = get_the_terms( $product_info, CS_TAG_TYPE );

		if ( user_can( $this->user_id, 'view_shop_reports' ) || $this->override ) {
			$product['stats']['total']['sales']              = cs_get_download_sales_stats( $product_info->ID );
			$product['stats']['total']['earnings']           = cs_get_download_earnings_stats( $product_info->ID );
			$product['stats']['monthly_average']['sales']    = cs_get_average_monthly_download_sales( $product_info->ID );
			$product['stats']['monthly_average']['earnings'] = cs_get_average_monthly_download_earnings( $product_info->ID );
		}

		if ( cs_has_variable_prices( $product_info->ID ) ) {
			foreach ( cs_get_variable_prices( $product_info->ID ) as $price ) {
				$product['pricing'][ sanitize_key( $price['name'] ) ] = $price['amount'];
			}
		} else {
			$product['pricing']['amount'] = cs_get_download_price( $product_info->ID );
		}

		if ( user_can( $this->user_id, 'view_shop_sensitive_data' ) || $this->override ) {
			foreach ( cs_get_download_files( $product_info->ID ) as $file ) {
				$product['files'][] = $file;
			}
			$product['notes'] = cs_get_product_notes( $product_info->ID );
		}

		return apply_filters( 'cs_api_products_product', $product );
	...
	public function get_stats( $args = array() ) {
		...
		if ( $args['type'] == 'sales' ) {
			if ( $args['product'] == null ) {
				if ( $args['date'] == null ) {
					...
					if ( $args['enddate'] < $args['startdate'] ) {
						$error['error'] = __( 'The end date must be later than the start date!', 'commercestore' );
					}

					// Ensure both the start and end date are specified
					if ( empty( $args['startdate'] ) || empty( $args['enddate'] ) ) {
						$error['error'] = __( 'Invalid or no date range specified!', 'commercestore' );
					}

					$start_date = $dates['year'] . '-' . $dates['m_start'] . '-' . $dates['day_start'];
					$end_date   = $dates['year_end'] . '-' . $dates['m_end'] . '-' . $dates['day_end'];

					$stats = CS()->payment_stats->get_sales_by_range( 'other', true, $start_date, $end_date );
					...
					$stats = CS()->payment_stats->get_sales_by_range( $args['date'], false, $start_date, $end_date );
					if ( $stats instanceof WP_Error ) {

						$error_message = __( 'There was an error retrieving earnings.', 'commercestore' );
			...
			return apply_filters( 'cs_api_stats_sales', $sales, $this );
			...
			if ( $args['product'] == null ) {
				if ( $args['date'] == null ) {
					...
					$earnings = CS()->payment_stats->get_earnings_by_range( 'other', true, $date_start, $date_end, $args['include_tax'] );
					...
					$results = CS()->payment_stats->get_earnings_by_range( $args['date'], false, $date_start, $date_end, $args['include_tax'] );
					if ( $results instanceof WP_Error ) {
						...
						$earnings['earnings'][ $args['date'] ] = cs_format_amount( $total_earnings );
					}
				}
			} elseif ( $args['product'] == 'all' ) {
				$products = get_posts( array( 'post_type' => CS_POST_TYPE, 'nopaging' => true ) );
				...
				$i = 0;
				foreach ( $products as $product_info ) {
					$earnings['earnings'][ $i ] = array( $product_info->post_name => cs_get_download_earnings_stats( $product_info->ID ) );
					$i ++;
				}
			} else {
				if ( get_post_type( $args['product'] ) == CS_POST_TYPE ) {
			...
			return apply_filters( 'cs_api_stats_earnings', $earnings, $this );
		} elseif ( $args['type'] == 'customers' ) {
			if ( version_compare( $cs_version, '2.3', '<' ) || ! cs_has_upgrade_completed( 'upgrade_customer_payments_association' ) ) {
				global $wpdb;
				$stats                                 = array();
				$count                                 = $wpdb->get_col( "SELECT COUNT(DISTINCT meta_value) FROM $wpdb->postmeta WHERE meta_key = '_cs_payment_user_email'" );
				$stats['customers']['total_customers'] = $count[0];

				return apply_filters( 'cs_api_stats_customers', $stats, $this );
			} else {
				$stats['customers']['total_customers'] = cs_count_customers();
			...
			return apply_filters( 'cs_api_stats', array( 'stats' => $stats, $this ) );
		}
	}
	...
	public function get_recent_sales() {
		...
		if ( isset( $wp_query->query_vars['id'] ) ) {
			$query   = array();
			$query[] = new CS_Payment( $wp_query->query_vars['id'] );
		} elseif ( isset( $wp_query->query_vars['purchasekey'] ) ) {
			$query   = array();
			$query[] = cs_get_payment_by( 'key', $wp_query->query_vars['purchasekey'] );
		} elseif ( isset( $wp_query->query_vars['email'] ) ) {
			$query = cs_get_payments( array(
				'fields'     => 'ids',
				'meta_key'   => '_cs_payment_user_email',
		...
		if ( $query ) {
			...
			foreach ( $query as $payment ) {
				if ( is_numeric( $payment ) ) {
					$payment = new CS_Payment( $payment );
				}
				...
				if ( ! empty( $payment->cart_details ) && is_array( $payment->cart_details ) ) {
					foreach ( $payment->cart_details as $key => $item ) {
						...
						if ( ! $price ) {
							...
							$price = cs_get_download_final_price( $item_id, $user_info, null );
						}
						...
						if ( isset( $item['item_number'] ) && isset( $item['item_number']['options'] ) ) {
							...
							if ( isset( $price_options['price_id'] ) ) {
								$price_name = cs_get_price_option_name( $item_id, $price_options['price_id'], $payment->ID );
							}
						}
		...
		return apply_filters( 'cs_api_sales', $sales, $this );
	}
	...
	public function get_discounts( $discount = null ) {
		...
		if ( empty( $discount ) ) {
			...
			if ( empty( $discounts ) ) {
				$error['error'] = __( 'No discounts found!', 'commercestore' );
			...
			$discount_object = cs_get_discount( $discount );
			if ( is_numeric( $discount ) && $discount_object ) {
				...
				$error['error'] = sprintf( __( 'Discount %s not found!', 'commercestore' ), $discount );

				return $error;
			}
		}

		return apply_filters( 'cs_api_discounts', $discount_list, $this );
	}
	...
	public function get_download_logs( $customer_id = 0 ) {
		...
		$logs = array();
		if ( ! $invalid_customer ) {
			$logs = $cs_logs->get_connected_logs( $query );
		}

		if ( empty( $logs ) ) {
			$error['error'] = __( 'No download logs found!', 'commercestore' );
		...
		foreach ( $logs as $log ) {
			...
			$payment_id = isset( $log_meta['_cs_log_payment_id'] ) ? $log_meta['_cs_log_payment_id'][0] : false;

			$payment_customer_id = cs_get_payment_customer_id( $payment_id );
			$payment_customer    = new CS_Customer( $payment_customer_id );
			$user_id             = ( $payment_customer->user_id > 0 ) ? $payment_customer->user_id : false;
			$ip                  = $log_meta['_cs_log_ip'][0];
			$files               = cs_get_payment_meta_downloads( $payment_id );
			$files               = cs_get_download_files( $files[0]['id'] );
			$file_id             = (int) $log_meta['_cs_log_file_id'][0];
			$file_id             = $file_id !== false ? $file_id : 0;
			$file_name           = isset( $files[ $file_id ]['name'] ) ? $files[ $file_id ]['name'] : null;

			$item = (array) apply_filters( 'cs_api_download_log_item', array(
				'ID'           => $log->ID,
				'user_id'      => $user_id,
		...
		return apply_filters( 'cs_api_download_logs', $downloads, $this );
	}
	...
	public function get_info() {
		...
		// Integrations
		if ( is_plugin_active( 'cs-commissions/cs-commissions.php' ) ) {
			$data['info']['integrations']['commissions'] = true;
		}

		if ( class_exists( 'CS_Software_Licensing' ) ) {
			$data['info']['integrations']['software_licensing'] = true;
		}

		if ( class_exists( 'CS_Front_End_Submissions' ) ) {
		...
		if ( user_can( $this->user_id, 'view_shop_sensitive_data' ) ) {
			...
			$data['info']['site']['cs_version'] = CS_VERSION;
		}

		$data['info']['site']['currency']            = cs_get_currency();
		$data['info']['site']['currency_position']   = cs_get_option( 'currency_position', 'before' );
		$data['info']['site']['decimal_separator']   = cs_get_option( 'decimal_separator', '.' );
		$data['info']['site']['thousands_separator'] = cs_get_option( 'thousands_separator', ',' );

		return apply_filters( 'cs_api_info', $data, $this );
	}
	...
	public function get_output_format() {
		...
		return apply_filters( 'cs_api_output_format', $format );
	}
	...
	private function log_request( $data = array() ) {
		...
		$query = array(
			'cs-api'     => $wp_query->query_vars['cs-api'],
	...
	public function output( $status_code = 200 ) {
		...
		do_action( 'cs_api_output_before', $this->data, $this, $format );

		switch ( $format ) :

			case 'xml' :

				require_once CS_PLUGIN_DIR . 'includes/libraries/class-ArrayToXML.php';
				$arraytoxml = new ArrayToXML();
				$xml        = $arraytoxml->buildXML( $this->data, 'cs' );
			...
			default :
				...
				do_action( 'cs_api_output_' . $format, $this->data, $this );

				break;

		endswitch;

		do_action( 'cs_api_output_after', $this->data, $this, $format );
	...
	function user_key_field( $user ) {
		if ( ( cs_get_option( 'api_allow_user_keys', false ) || current_user_can( 'manage_shop_settings' ) ) && current_user_can( 'edit_user', $user->ID ) ) {
			...
			<table class="form-table">
				<tbody>
				<tr>
					<th><?php _e( 'Downloads API Keys', 'commercestore' ); ?></th>
					<td>
						<?php
						$public_key = $this->get_user_public_key( $user->ID );
						$secret_key = $this->get_user_secret_key( $user->ID );

						if ( empty( $user->cs_user_public_key ) ) { ?>
							<p class="description">
								<label>
						...
						<?php } else { ?>
							<strong style="display:inline-block; width: 125px;"><?php _e( 'Public key:', 'commercestore' ); ?>&nbsp;</strong>
                            <input type="text" disabled="disabled" class="regular-text" id="publickey" value="<?php echo esc_attr( $public_key ); ?>"/>
							<br/>
							<strong style="display:inline-block; width: 125px;"><?php _e( 'Secret key:', 'commercestore' ); ?>&nbsp;</strong>
                            <input type="text" disabled="disabled" class="regular-text" id="privatekey" value="<?php echo esc_attr( $secret_key ); ?>"/>
							<br/>
							<strong style="display:inline-block; width: 125px;"><?php _e( 'Token:', 'commercestore' ); ?>&nbsp;</strong>
                            <input type="text" disabled="disabled" class="regular-text" id="token" value="<?php echo esc_attr( $this->get_token( $user->ID ) ); ?>"/>
							<br/>
							<input name="cs_set_api_key" type="checkbox" id="cs_set_api_key" value="0"/>
							<span class="description"><label for="cs_set_api_key"><?php _e( 'Revoke API Keys', 'commercestore' ); ?></label></span>
						<?php } ?>
					</td>
				</tr>
				</tbody>
	...
	public function process_api_key( $args ) {
		if ( ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'cs-api-nonce' ) ) {
			wp_die( __( 'Nonce verification failed', 'commercestore' ), __( 'Error', 'commercestore' ), array( 'response' => 403 ) );
		}

		if ( empty( $args['user_id'] ) ) {
			wp_die( __( 'User ID Required', 'commercestore' ), __( 'Error', 'commercestore' ), array( 'response' => 401 ) );
		}
		...
		$process = isset( $args['cs_api_process'] ) ? strtolower( $args['cs_api_process'] ) : false;

		if ( $user_id == get_current_user_id() && ! cs_get_option( 'allow_user_api_keys' ) && ! current_user_can( 'manage_shop_settings' ) ) {
			wp_die( sprintf( __( 'You do not have permission to %s API keys for this user', 'commercestore' ), $process ), __( 'Error', 'commercestore' ), array( 'response' => 403 ) );
	...
	public function revoke_api_key( $user_id = 0 ) {
		...
		if ( ! empty( $public_key ) ) {
			delete_transient( md5( 'cs_api_user_' . $public_key ) );
			delete_transient( md5( 'cs_api_user_public_key' . $user_id ) );
			delete_transient( md5( 'cs_api_user_secret_key' . $user_id ) );
	...
	public function update_key( $user_id ) {
		cs_update_user_api_key( $user_id );
	}
	...
	private function get_default_sales_stats() {
		...
		$sales['sales']['totals']        = cs_get_total_sales();
	...
	private function get_default_earnings_stats( $args ) {
		...
		$earnings['earnings']['totals']        = cs_get_total_earnings( $args['include_tax'] );
	...
	public function api_key_backwards_copmat( $check, $object_id, $meta_key, $single ) {
		...
		if ( $meta_key !== 'cs_user_public_key' && $meta_key !== 'cs_user_secret_key' ) {
			return $check;
		}
		...
		switch ( $meta_key ) {
			case 'cs_user_public_key':
				$return = CS()->api->get_user_public_key( $object_id );
				break;
			case 'cs_user_secret_key':
				$return = CS()->api->get_user_secret_key( $object_id );
				break;
		}
	...
	public function log_requests() {
		return apply_filters( 'cs_api_log_requests', true );
	}
