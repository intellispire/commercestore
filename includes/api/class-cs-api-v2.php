 ...
 * @since  2.6
 */
class CS_API_V2 extends CS_API_V1 {
	...
	public function get_products( $args = array() ) {
		...
		if ( empty( $args['product'] ) ) {
			...
			switch ( $query_args['orderby'] ) {
				...
				case 'earnings':
					if ( user_can( $this->user_id, 'view_shop_sensitive_data' ) || current_user_can( 'view_shop_sensitive_data' ) || $this->override ) {
						$query_args['meta_key'] = '_cs_download_earnings';
						$query_args['orderby']  = 'meta_value_num';
					}
					break;
		...
		return apply_filters( 'cs_api_products', $products );
	}
	...
	public function get_product_data( $product_info ) {
		...
		if ( cs_use_skus() ) {
			$product['info']['sku'] = cs_get_download_sku( $product['info']['id'] );
		}

		return apply_filters( 'cs_api_products_product_v2', $product );
	...
	public function get_customers( $args = array() ) {
		...
		if( $customer_query ) {
			...
			foreach ( $customer_query as $customer_obj ) {
				// Setup a new CS_Customer object so additional details are defined (like additional emails)
				$customer_obj = new CS_Customer( $customer_obj->id );
			...
			$error['error'] = sprintf( __( 'Customer %s not found!', 'commercestore' ), $args['customer'] );
			return $error;
	...
	public function get_recent_sales() {
		...
		if( isset( $wp_query->query_vars['id'] ) ) {
			...
			$query = cs_get_payments( array( 'fields' => 'ids', 'meta_key' => '_cs_payment_user_email', 'meta_value' => $wp_query->query_vars['email'], 'number' => $this->per_page(), 'page' => $this->get_paged(), 'status' => 'complete' ) );
		} else {
			$query = cs_get_payments( array( 'fields' => 'ids', 'number' => $this->per_page(), 'page' => $this->get_paged(), 'status' => 'complete' ) );
		}
			...
			foreach ( $query as $payment ) {
				...
				foreach ( $payment->cart_details as $key => $item ) {
					...
					foreach ( $discount_values as $discount => $amount ) {
						...
						$item_discount = cs_get_cart_item_discount_amount( $item, $discount );
