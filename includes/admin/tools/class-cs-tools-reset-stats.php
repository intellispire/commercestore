...
/**
 * CS_Tools_Reset_Stats class.
 *
 * @since 2.5
 * @since 3.0 Updated to work with new query methods.
 */
class CS_Tools_Reset_Stats extends CS_Batch_Export {
	...
	public function get_data() {
		...
		$tables = $this->get_stored_data( 'cs_reset_tables_to_truncate' );
		...
		if ( $step_items ) {
			...
			cs_debug_log( var_export($query, true), true );
			$wpdb->query( $query );
	...
	public function get_percentage_complete() {
		$items = $this->get_stored_data( 'cs_reset_tables_to_truncate' );
		$total = count( $items );
	...
	public function process_step() {
		if ( ! $this->can_export() ) {
			wp_die( esc_html__( 'You do not have permission to export data.', 'commercestore' ), esc_html__( 'Error', 'commercestore' ), array( 'response' => 403 ) );
		}
		...
		if ( $had_data ) {
			...
			update_option( 'cs_earnings_total', 0 );
			...
			$this->delete_data( 'cs_reset_tables_to_truncate' );

			// Reset the sequential order numbers
			if ( cs_get_option( 'enable_sequential' ) ) {
				delete_option( 'cs_last_payment_number' );
			}

			$this->done    = true;
			$this->message = __( 'Your store has been successfully reset.', 'commercestore' );
			return false;
		}
	}
	...
	public function pre_fetch() {
		...
		$tables = get_option( 'cs_reset_tables_to_truncate', false );

		if ( false === $tables ) {
			$tables = array();

			foreach ( CS()->components as $component ) {
				/** @var $component CS\Component */
			...
			$tables = apply_filters( 'cs_reset_tables_to_truncate', $tables );

			$this->store_data( 'cs_reset_tables_to_truncate', $tables );
		}
	}
