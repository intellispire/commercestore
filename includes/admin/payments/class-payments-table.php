...
/**
 * CS_Payment_History_Table Class
 *
 ...
 */
class CS_Payment_History_Table extends List_Table {
	...
	public function __construct() {
		...
		$types = array_keys( cs_get_order_types() );
	...
	private function set_base_url() {
		...
		$this->base_url = cs_get_admin_url( array(
	...
	private function filter_bar_hooks() {
		add_action( 'cs_admin_filter_bar_orders',       array( $this, 'filter_bar_items'     ) );
		add_action( 'cs_after_admin_filter_bar_orders', array( $this, 'filter_bar_searchbox' ) );
	}
	...
	public function filter_bar_items() {
		...
		// Filters
		$all_modes    = cs_get_payment_modes();
		$all_gateways = cs_get_payment_gateways();
		...
		// Add "All" and pluck labels
			...
			$modes = array_merge( array(
				'all' => __( 'All modes', 'commercestore' )
		...
		// Add "All" and pluck labels
			...
			$gateways = array_merge( array(
				'all' => __( 'All gateways', 'commercestore' )
		 ...
		 * @since 2.8.11
		 */
		$gateways = apply_filters( 'cs_payments_table_gateways', $gateways );

		// Output the items
		if ( ! empty( $modes ) ) : ?>

			<span id="cs-mode-filter">
		...
		<span id="cs-advanced-filters">
			<input type="button" class="button cs-advanced-filters-button button-secondary" value="<?php esc_html_e( 'More', 'commercestore' ); ?>"/>

			<div class="inside">
				<fieldset>
					<legend for="order-amount-filter-type"><?php esc_html_e( 'Total is', 'commercestore' ); ?></legend>
					<?php
					$options = array(
						'eq' => __( 'equal to', 'commercestore' ),
						'gt' => __( 'greater than', 'commercestore' ),
						'lt' => __( 'less than', 'commercestore' ),
					);
				...
				<fieldset>
					<legend><?php esc_html_e( 'Product', 'commercestore' ); ?></legend>
					<?php
				...
				<fieldset>
					<legend><?php esc_html_e( 'Country & Region', 'commercestore' ); ?></legend>
					<?php
				...
				if ( has_action( 'cs_payment_advanced_filters_after_fields' ) ) : ?>

					<fieldset class="cs-add-on-filters">
						<legend><?php esc_html_e( 'Extras', 'commercestore' ); ?></legend>

						<?php do_action( 'cs_payment_advanced_filters_after_fields' ); ?>
	...
	public function filter_bar_searchbox() {
		do_action( 'cs_payment_advanced_filters_row' );

		$this->search_box( esc_html__( 'Search', 'commercestore' ), 'cs-payments' );
	}
	...
	public function search_box( $text, $input_id ) {
		...
		<p class="search-form">
			<?php do_action( 'cs_payment_history_search' ); ?>
			<label class="screen-reader-text" for="<?php echo esc_attr( $input_id ); ?>"><?php echo esc_html( $text ); ?>:</label>
			<input type="search" id="<?php echo esc_attr( $input_id ); ?>" name="s" placeholder="<?php esc_html_e( 'Search orders...', 'commercestore' ); ?>" value="<?php _admin_search_query(); ?>" />
		</p>
	...
	public function no_items() {
		if ( ! cs_has_upgrade_completed( 'migrate_orders' ) ) {
			...
			if ( ! empty( $orders ) ) {
				esc_html_e( 'CommerceStore needs to upgrade the database. Orders will be available when that has completed.', 'commercestore' );
				return;
			}
		}
		esc_html_e( 'No orders found.', 'commercestore' );
	}
	...
	public function get_columns() {
		 ...
		 */
		$columns = apply_filters( 'cs_payments_table_columns', $columns );
	...
	public function get_sortable_columns() {
		return apply_filters( 'cs_payments_table_sortable_columns', array(
	 ...
	 *
	 * @since 1.4
	 * @since 3.0 Updated to use the new CS\Orders\Order class.
	 *
	 * @param CS\Orders\Order $order       Order object.
	...
	public function column_default( $order, $column_name ) {
		...
		switch ( $column_name ) {
			case 'amount':
				$value = cs_display_amount( $order->total, $order->currency );
				break;
			case 'date':
				$value = '<time datetime="' . esc_attr( CS()->utils->date( $order->date_created, null, true )->toDateTimeString() ) . '">' . cs_date_i18n( $order->date_created, 'M. d, Y' ) . '<br>' . cs_date_i18n( strtotime( $order->date_created ), 'H:i' ) . ' ' . $timezone_abbreviation . '</time>';
				break;
			case 'gateway':
				$value = cs_get_gateway_admin_label( $order->gateway );
				...
				break;
			case 'status':
				$value = cs_get_order_status_badge( $order->status );
				break;
			default:
		...
		return apply_filters( 'cs_payments_table_column', $value, $order->id, $column_name );
	}
	...
	public function column_cb( $order ) {
		...
		return sprintf(
			...
			esc_html( sprintf( __( 'Select %s', 'commercestore' ), $order_number ) )
		);
	}
	...
	public function column_number( $order ) {
		...
		// Default row actions
		$row_actions = array(
			'view' => '<a href="' . esc_url( $view_url ) . '">' . esc_html__( 'Edit', 'commercestore' ) . '</a>',
		);
		...
		if ( 'sale' === $this->type && 'complete' === $order->status && ! empty( $order->email ) ) {
			$row_actions['email_links'] = '<a href="' . esc_url( add_query_arg( array(
				...
				), $this->base_url ) ) . '">' . __( 'Resend Receipt', 'commercestore' ) . '</a>';
		}

		// Keep Delete at the end
		if ( cs_is_order_trashable( $order->id ) ) {
			$trash_url = wp_nonce_url( add_query_arg( array(
				'cs-action'  => 'trash_order',
				'purchase_id' => $order->id,
			), $this->base_url ), 'cs_payment_nonce' );
			$row_actions['trash'] = '<a href="' . esc_url( $trash_url ) . '">' . esc_html__( 'Trash', 'commercestore' ) . '</a>';
		} elseif ( cs_is_order_restorable( $order->id ) ) {
			$restore_url = wp_nonce_url( add_query_arg( array(
				'cs-action'  => 'restore_order',
				'purchase_id' => $order->id,
			), $this->base_url ), 'cs_payment_nonce' );
			$row_actions['restore'] = '<a href="' . esc_url( $restore_url ) . '">' . esc_html__( 'Restore', 'commercestore' ) . '</a>';

			$delete_url = wp_nonce_url( add_query_arg( array(
				'cs-action'  => 'delete_order',
				'purchase_id' => $order->id,
			), $this->base_url ), 'cs_payment_nonce' );
			$row_actions['delete'] = '<a href="' . esc_url( $delete_url ) . '">' . esc_html__( 'Delete Permanently', 'commercestore' ) . '</a>';

			unset( $row_actions['view'] );
		}

		if ( has_filter( 'cs_payment_row_actions' ) ) {
			 ...
			 * @param CS_Payment|false $payment
			 */
			$row_actions = apply_filters_deprecated( 'cs_payment_row_actions', array( $row_actions, $payment ), '3.0', 'cs_order_row_actions' );
		}
		 ...
		 * @since 3.0
		 */
		$row_actions = apply_filters( 'cs_order_row_actions', $row_actions, $order );
		...
		// Primary link
		$order_number = 'sale' === $order->type ? $order->get_number() : $order->order_number;
		$link         = cs_is_order_restorable( $order->id ) ? '<span class="row-title">' . esc_html( $order_number ) . '</span>' : '<a class="row-title" href="' . esc_url( $view_url ) . '">' . esc_html( $order_number ) . '</a>';
	...
	public function column_customer( $order ) {
		 ...
		 */
		$name = apply_filters( 'cs_payments_table_column', $name, $order->id, 'email' );
	...
	public function get_bulk_actions() {
		if ( 'refund' !== $this->type ) {
			$action = array(
				'set-status-complete'     => __( 'Mark Completed',   'commercestore' ),
				'set-status-pending'     => __( 'Mark Pending',     'commercestore' ),
				'set-status-processing'  => __( 'Mark Processing',  'commercestore' ),
				'set-status-revoked'     => __( 'Mark Revoked',     'commercestore' ),
				'set-status-failed'      => __( 'Mark Failed',      'commercestore' ),
				'set-status-abandoned'   => __( 'Mark Abandoned',   'commercestore' ),
				'set-status-preapproval' => __( 'Mark Preapproved', 'commercestore' ),
				'set-status-cancelled'   => __( 'Mark Cancelled',   'commercestore' ),
				'resend-receipt'         => __( 'Resend Receipts', 'commercestore' ),
			);
		} else {
			$action = array();
		}
		...
		if ( 'trash' === $this->get_status() ) {
			$action = array(
				'restore' => __( 'Restore', 'commercestore' ),
			);
		} else {
			$action['trash'] = __( 'Move to Trash', 'commercestore' );
		}

		return apply_filters( 'cs_payments_table_bulk_actions', $action );
	}
	 ...
	 *
	 * @see cs_orders_list_table_process_bulk_actions()
	 */
	public function process_bulk_action() {
		_doing_it_wrong( __FUNCTION__, 'Orders list table bulk actions are now handled by cs_orders_list_table_process_bulk_actions(). Please do not call this method directly.', 'CS 3.0' );
	}
	...
	public function get_payment_counts() {
		...
		$this->counts = cs_get_order_counts( $args );
	}
	...
	public function payments_data() {
		_cs_deprecated_function( __METHOD__, '3.0', 'CS_Payment_History_Table::get_data()' );
	...
	public function get_data() {
		...
		// Force CS\Orders\Order objects to be returned
	...
	public function get_views() {
		 ...
		 */
		$views = apply_filters( 'cs_payments_table_views', $views );
	...
	private function parse_args( $paginate = true ) {
		 ...
		 *
		 * @see   https://github.com/commercestore/commercestore/issues/6063
		 * @since 2.8.11
		 *
		 * @param string $gateway
		 */
		$gateway = apply_filters( 'cs_payments_table_search_gateway', $gateway );
