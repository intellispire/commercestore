...
class Refund_Items_Table extends List_Table {
	...
	public function get_columns() {
		$columns = array(
			...
			'subtotal' => __( 'Subtotal', 'commercestore' ),
		);
	...
	private function get_quantity_column( $item, $column_name, $item_id, $object_type ) {
		...
		?>
		<div class="cs-form-group">
			<label for="cs_order_item_quantity_<?php echo esc_attr( $item_id ); ?>" class="screen-reader-text">
			<?php esc_html_e( 'Quantity to refund', 'commercestore' ); ?>
		</label>
			<div class="cs-form-group__control">
				<?php if ( 'order_item' !== $object_type ) : ?>
					<input type="hidden" data-original="<?php echo esc_attr( $item_quantity ); ?>" id="cs_order_item_quantity_<?php echo esc_attr( $item_id ); ?>" class="cs-order-item-refund-quantity cs-order-item-refund-input readonly" name="refund_<?php echo esc_attr( $object_type ); ?>[<?php echo esc_attr( $item->id ); ?>][quantity]" value="<?php echo esc_attr( $item_quantity ); ?>" disabled />
				<?php else : ?>
					<?php
					...
					$args = array(
						...
						'id'               => 'cs-order-item-quantity-' . esc_attr( $item_id ),
						'class'            => 'cs-order-item-refund-quantity cs-order-item-refund-input',
						'disabled'         => true,
	...
	public function column_cb( $item ) {
		...
		if ( 'refunded' !== $item->status && 0.00 != $total_remaining ) {
			...
			return sprintf(
				'<input type="checkbox" name="%1$s[]" id="%1$s-%2$s" class="cs-order-item-refund-checkbox" value="%2$s" /><label for="%1$s-%2$s" class="screen-reader-text">%3$s</label>',
				/*$1%s*/
