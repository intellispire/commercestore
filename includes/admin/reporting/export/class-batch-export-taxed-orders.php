 ...
 * @since 3.0
 */
class CS_Batch_Taxed_Orders_Export extends CS_Batch_Export {
	...
	public function csv_cols() {
		$cols = array(
			'id'           => __( 'Order ID', 'commercestore' ), // unaltered payment ID (use for querying)
			'seq_id'       => __( 'Order Number', 'commercestore' ), // sequential payment ID
			...
			'amount'       => __( 'Amount', 'commercestore' ) . ' (' . html_entity_decode( cs_currency_filter( '' ) ) . ')',
			'tax'          => __( 'Tax', 'commercestore' ) . ' (' . html_entity_decode( cs_currency_filter( '' ) ) . ')',
	...
	public function get_data() {
		...
		foreach ( $orders as $order ) {
			...
			foreach ( $items as $key => $item ) {
				...
				$products .= html_entity_decode( cs_currency_filter( cs_format_amount( $price ) ) );
			...
			$data[] = array(
				...
				'mode'         => $order->mode,
				'status'       => $order->status,
				'country_name' => isset( $user_info['address']['country'] ) ? cs_get_country_name( $user_info['address']['country'] ) : '',
			);
		}
	...
	public function query_clauses( $clauses, $base ) {
		...
		$clauses['where'] = ! empty( $clauses['where'] )
			? $clauses['where'] .= ' AND cs_o.tax > 0'
			: 'cs_o.tax > 0';

		if ( ! empty( $this->country ) ) {
			$clauses['join'] = " INNER JOIN {$wpdb->cs_order_addresses} cs_oa ON cs_o.id = cs_oa.order_id";
			$clauses['where'] .= $wpdb->prepare( ' AND cs_oa.country = %s', $this->country );
		}

		if ( ! empty( $this->region ) ) {
			$clauses['where'] .= $wpdb->prepare( ' AND cs_oa.region = %s', $this->region );
		}
