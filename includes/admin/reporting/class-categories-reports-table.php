...
/**
 * CS_Categories_Reports_Table Class
 *
 * Renders the Download Reports table
 *
 * @since 2.4
 */
class CS_Categories_Reports_Table extends List_Table {
	...
	public function get_columns() {
		return array(
			'label'          => __( 'Category',             'commercestore' ),
			'total_sales'    => __( 'Total Sales',          'commercestore' ),
			'total_earnings' => __( 'Total Earnings',       'commercestore' ),
			'avg_sales'      => __( 'Monthly Sales Avg',    'commercestore' ),
			'avg_earnings'   => __( 'Monthly Earnings Avg', 'commercestore' )
		);
	}
	...
	public function reports_data() {
		_cs_deprecated_function( __METHOD__, '3.0', 'CS_Categories_Reports_Table::get_data()' );
	...
	public function get_data() {
		...
		if ( ! empty( $dates[ 'year' ] ) ) {
			...
			$cached_report_key = 'cs_earnings_by_category_data' . $start_date . '_' . $end_date;
		} else {
			$start_date        = false;
			$end_date          = false;
			$cached_report_key = 'cs_earnings_by_category_data';
		}
		...
		if ( false !== $cached_reports ) {
			...
			foreach ( $categories as $category ) {
				...
				foreach ( $downloads as $download ) {
					$current_sales    = CS()->payment_stats->get_sales( $download, $start_date, $end_date );
					$current_earnings = CS()->payment_stats->get_earnings( $download, $start_date, $end_date, $include_taxes );

					$current_average_sales = cs_get_average_monthly_download_sales( $download );
					$current_average_earnings = cs_get_average_monthly_download_earnings( $download );
				...
				$avg_earnings = round( $avg_earnings, cs_currency_decimal_filter() );
				if ( ! empty( $avg_earnings ) && $avg_sales < 1 ) {
					$avg_sales = __( 'Less than 1', 'commercestore' );
				} else {
					$avg_sales = round( cs_format_amount( $avg_sales, false ) );
				}

				$reports_data[] = array(
					'ID'                 => $category->term_id,
					'label'              => $category->name,
					'total_sales'        => cs_format_amount( $sales, false ),
					'total_sales_raw'    => $sales,
					'total_earnings'     => cs_currency_filter( cs_format_amount( $earnings ) ),
					'total_earnings_raw' => $earnings,
					'avg_sales'          => $avg_sales,
					'avg_earnings'       => cs_currency_filter( cs_format_amount( $avg_earnings ) ),
					'is_child'           => false,
				);
				...
				if ( ! empty( $child_terms ) ) {
					foreach ( $child_terms as $child_term ) {
						...
						foreach ( $child_downloads as $child_download ) {
							$current_average_sales    = $current_sales    = CS()->payment_stats->get_sales( $child_download, $start_date, $end_date );
							$current_average_earnings = $current_earnings = CS()->payment_stats->get_earnings( $child_download, $start_date, $end_date );
						...
						$child_avg_earnings = round( $child_avg_earnings / count( $child_downloads ), cs_currency_decimal_filter() );

						$reports_data[] = array(
							'ID'                 => $child_term->term_id,
							'label'              => '&#8212; ' . $child_term->name,
							'total_sales'        => cs_format_amount( $child_sales, false ),
							'total_sales_raw'    => $child_sales,
							'total_earnings'     => cs_currency_filter( cs_format_amount( $child_earnings ) ),
							'total_earnings_raw' => $child_earnings,
							'avg_sales'          => cs_format_amount( $child_avg_sales, false ),
							'avg_earnings'       => cs_currency_filter( cs_format_amount( $child_avg_earnings ) ),
							'is_child'           => true
						);
					}
				}
	...
	public function output_sales_graph() {
		...
		if ( empty( $total_sales ) ) {
			echo '<p><em>' . __( 'No sales for dates provided.', 'commercestore' ) . '</em></p>';
		}

		// Sort High to Low, prior to filter so people can reorder if they please
		arsort( $data );
		$data = apply_filters( 'cs_category_sales_graph_data', $data );

		$options = apply_filters( 'cs_category_sales_graph_options', array(
			'legend_formatter' => 'csLegendFormatterSales',
		), $data );

		$pie_graph = new CS_Pie_Graph( $data, $options );
		$pie_graph->display();
	}
	...
	public function output_earnings_graph() {
		...
		if ( empty( $total_earnings ) ) {
			echo '<p><em>' . __( 'No earnings for dates provided.', 'commercestore' ) . '</em></p>';
		}

		// Sort High to Low, prior to filter so people can reorder if they please
		arsort( $data );
		$data = apply_filters( 'cs_category_earnings_graph_data', $data );

		$options = apply_filters( 'cs_category_earnings_graph_options', array(
			'legend_formatter' => 'csLegendFormatterEarnings',
		), $data );
	 ...
	 *
	 * @since 2.4
	 * @uses CS_Categories_Reports_Table::get_columns()
	 * @uses CS_Categories_Reports_Table::get_sortable_columns()
	 * @uses CS_Categories_Reports_Table::reports_data()
	 * @return void
	 */
