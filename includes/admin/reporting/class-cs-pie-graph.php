 ...
 * @since 2.4
 */
class CS_Pie_Graph extends CS_Graph {
	...
	*/
	...
	public function __construct( $_data, $options = array() ) {
		...
		$this->options = wp_parse_args( $options,  array(
			...
			'label_formatter'   => 'csLabelFormatter',
		...
		add_action( 'cs_graph_load_scripts', array( $this, 'load_additional_scripts' ) );
	}
	...
	public function load_additional_scripts() {
		wp_enqueue_script( 'cs-jquery-flot-pie', CS_PLUGIN_URL . 'assets/js/vendor/jquery.flot.pie.min.js' );
	}
		...
		if ( count( $this->data ) ) {
			...
			<script type="text/javascript">
				...
				jQuery( document ).ready( function($) {
					var <?php echo $this->id; ?>Chart = $('#cs-pie-graph-<?php echo $this->id; ?>');
					...
					$(<?php echo $this->id; ?>Chart).on('plothover', function (event, pos, item) {
						$('.cs-legend-item-wrapper').css('background-color', 'inherit');
						if ( item ) {
							var label = item.series.label;
							var id    = item.series.cs_vars.id;

							var slug = label.toLowerCase().replace(/\s/g, '-');
							var legendTarget = '#' + id + slug;

							$('.cs-legend-item-wrapper' + legendTarget).css('background-color', '#f0f0f0');
						}
					});
					<?php endif; ?>
				});
			...
			<div class="cs-pie-graph-wrap">
				<div id="cs-pie-graph-<?php echo $this->id; ?>" class="cs-pie-graph" style="height: <?php echo $this->options['height']; ?>px;"></div>
				<div id="cs-pie-legend-<?php echo $this->id; ?>" class="cs-pie-legend"></div>
			</div>
			<?php
		}
		return apply_filters( 'cs_pie_graph_output', ob_get_clean(), $this->id, $this->data, $this->options );
