...
class CS_Graph {
	...
	*/
	...
	public function get_data() {
		return apply_filters( 'cs_get_graph_data', $this->data, $this );
	}
	...
	public function load_scripts() {
		wp_enqueue_script( 'cs-jquery-flot',      CS_PLUGIN_URL . 'assets/js/vendor/jquery.flot.min.js' );
		wp_enqueue_script( 'cs-jquery-flot-time', CS_PLUGIN_URL . 'assets/js/vendor/jquery.flot.time.min.js' );
		 ...
		 * @since 1.9
		 */
		do_action( 'cs_graph_load_scripts' );
	}
...
?>
		<script type="text/javascript">
			jQuery( document ).ready( function($) {
				...
				function cs_flot_tooltip(x, y, contents) {
					$('<div id="cs-flot-tooltip">' + contents + '</div>').css( {
						position: 'absolute',
						display: 'none',
						top: y + 5,
						left: x + 5,
				...
				$("#cs-graph-<?php echo $this->id; ?>").bind("plothover", function (event, pos, item) {
					$("#x").text(pos.x.toFixed(2));
					$("#y").text(pos.y.toFixed(2));
					if (item) {
						if (previousPoint != item.dataIndex) {
							previousPoint = item.dataIndex;
							$("#cs-flot-tooltip").remove();
							var x = item.datapoint[0].toFixed(2),
							y = item.datapoint[1].toFixed(2);
							if( item.series.id.includes('earnings') ) {
								if( cs_vars.currency_pos == 'before' ) {
									cs_flot_tooltip( item.pageX, item.pageY, item.series.label + ' ' + cs_vars.currency_sign + y );
								} else {
									cs_flot_tooltip( item.pageX, item.pageY, item.series.label + ' ' + y + cs_vars.currency_sign );
								}
							} else {
								cs_flot_tooltip( item.pageX, item.pageY, item.series.label + ' ' + y.replace( '.00', '' ) );
							}
						}
					} else {
						$("#cs-flot-tooltip").remove();
		...
		<div id="cs-graph-<?php echo $this->id; ?>" class="cs-graph" style="height: 300px;"></div>
<?php
		return ob_get_clean();
	}
	...
	public function display() {
		do_action( 'cs_before_graph', $this );
		echo $this->build_graph();
		do_action( 'cs_after_graph', $this );
	}
