 ...
 * @since       3.0
 */
namespace CS\Reports\Data\Taxes;
...
class Tax_Collected_By_Location extends List_Table {
	...
	public function get_columns() {
		return array(
			'country'  => __( 'Country/Region', 'commercestore' ),
			'from'     => __( 'From', 'commercestore' ),
			'to'       => __( 'To', 'commercestore' ),
			'gross'    => __( 'Gross', 'commercestore' ),
			'tax'      => __( 'Tax', 'commercestore' ),
			'net'      => __( 'Net', 'commercestore' ),
 		);
	}
	...
	public function get_data() {
		...
		$data             = array();
		$tax_rates        = cs_get_tax_rates( array(), OBJECT );
		...
		$format_currency  = $convert_currency ? cs_get_currency() : strtoupper( $currency );

		// Date query.
		$date_query  = '';

		if ( ! empty( $date_range['start'] ) && '0000-00-00 00:00:00' !== $date_range['start'] ) {
			$date_query .= $wpdb->prepare( " AND {$wpdb->cs_orders}.date_created >= %s", esc_sql( CS()->utils->date( $date_range['start'], null, false )->startOfDay()->format( 'mysql' ) ) );
		}

		if ( ! empty( $date_range['end'] ) && '0000-00-00 00:00:00' !== $date_range['end'] ) {
			$date_query .= $wpdb->prepare( " AND {$wpdb->cs_orders}.date_created <= %s", esc_sql( CS()->utils->date( $date_range['end'], null, false )->endOfDay()->format( 'mysql' ) ) );
		}

		$from = empty( $date_range['start'] ) || '0000-00-00 00:00:00' === $date_range['start']
				? '&mdash;'
				: cs_date_i18n( CS()->utils->date( $date_range['start'], null, false )->startOfDay()->timestamp );

		$to = empty( $date_range['end'] ) || '0000-00-00 00:00:00' === $date_range['end']
			? '&mdash;'
			: cs_date_i18n( CS()->utils->date( $date_range['end'], null, false )->endOfDay()->timestamp );

		$tax_column   = $convert_currency ? 'tax / rate' : 'tax';
		$total_column = $convert_currency ? 'total / rate' : 'total';

		$currency_sql = '';
		if ( ! $convert_currency && array_key_exists( strtoupper( $currency ), cs_get_currencies() ) ) {
		...
		foreach ( $tax_rates as $tax_rate ) {
			...
			$location = cs_get_country_name( $tax_rate->name );

			if ( ! empty( $tax_rate->description ) ) {
				$location .= ' &mdash; ' . cs_get_state_name( $tax_rate->name, $tax_rate->description );
			}
			...
			$results = $wpdb->get_results( $wpdb->prepare( "
				...
				FROM {$wpdb->cs_orders}
				INNER JOIN {$wpdb->cs_order_addresses} ON {$wpdb->cs_order_addresses}.order_id = {$wpdb->cs_orders}.id
				WHERE {$wpdb->cs_order_addresses}.country = %s {$region} {$date_query} {$currency_sql}
			...
			$data[ $country_region ] = array(
				...
				'from'     => $from,
				'to'       => $to,
				'gross'    => cs_currency_filter( cs_format_amount( floatval( $results[0]['total'] ) ), $format_currency ),
				'tax'      => cs_currency_filter( cs_format_amount( floatval( $results[0]['tax'] ) ), $format_currency ),
				'net'      => cs_currency_filter( cs_format_amount( floatval( $results[0]['total'] - $results[0]['tax'] ) ), $format_currency ),
			);
		}
		...
		if( $all_orders[0]['total'] > 0 && $all_orders[0]['tax'] > 0 ) {
			...
			$data[ 'global' ] = array(
				'country'  => __( 'Global Rate', 'commercestore' ),
				'from'     => $from,
				'to'       => $to,
				'gross'    => cs_currency_filter( cs_format_amount( floatval( max( 0, $all_orders[0]['total'] ) ) ), $format_currency ),
				'tax'      => cs_currency_filter( cs_format_amount( floatval( max( 0, $all_orders[0]['tax'] ) ) ), $format_currency ),
				'net'      => cs_currency_filter( cs_format_amount( floatval( max( 0, $all_orders[0]['total'] - $all_orders[0]['tax'] ) ) ), $format_currency ),
			);
