...
class CS_Payment {
	 ...
	 *
	 * @since 2.5
	 * @since 3.0 Updated to fetch transaction ID from cs_ordermeta table.
	 *
	...
	private function insert_payment() {
		if ( empty( $this->key ) ) {
			...
			$this->key            = strtolower( md5( $this->email . date( 'Y-m-d H:i:s' ) . $auth_key . uniqid( 'cs', true ) ) );  // Unique key
		...
		$order_id = cs_add_order( $order_args );
		...
		if ( ! empty( $order_id ) ) {
			...
			/**
			 * This run of the cs_payment_meta filter is for backwards compatibility purposes. The filter will also run
			 * in the CS_Payment::save method. By keeping this here, it retains compatibility of adding payment meta
			 * prior to the payment being inserted, as was previously supported by cs_insert_payment().
			 *
			 * @reference: https://github.com/commercestore/commercestore/issues/5838
			 */
			$this->payment_meta = apply_filters( 'cs_payment_meta', $this->payment_meta, $payment_data );
			if ( ! empty( $this->payment_meta['fees'] ) ) {
				...
				foreach ( $this->fees as $key => $fee ) {
					...
					$tax = ( isset( $fee['no_tax'] ) && false === $fee['no_tax'] ) || $fee['amount'] < 0
						? floatval( cs_calculate_tax( $fee['amount'] ) )
						: 0.00;
			...
			if ( cs_get_option( 'enable_sequential' ) ) {
				...
				$this->number = cs_format_payment_number( $number );

				$this->update_meta( '_cs_payment_number', $this->number );
				$order_data['order_number'] = $this->number;

				update_option( 'cs_last_payment_number', $number );
			}

			cs_update_order( $order_id, $order_data );

			$this->update_meta( '_cs_payment_meta', $this->payment_meta );
			...
			foreach ( $order_meta as $key => $value ) {
				if ( ! empty( $value ) ) {
					cs_add_order_meta( $order_id, $key, $value );
				}
			}
	...
	public function save() {
		...
		if ( ! empty( $this->pending ) ) {
				...
				switch ( $key ) {
					...
					case 'discounts':
						...
						foreach ( $this->discounts as $discount ) {
							/** @var CS_Discount $discount_obj */
							$discount_obj = cs_get_discount_by( 'code', $discount );
							$args         = array(
								'object_id'   => $this->ID,
								'object_type' => 'order',
					...
					case 'tax_rate':
						...
						$this->update_meta( '_cs_payment_tax_rate', $tax_rate );
						break;
					...
					default:
						 ...
						 */
						do_action( 'cs_payment_save', $this, $key );
						break;
				}
			}
			...
			// Do some merging of user_info before we merge it all, to honor the cs_payment_meta filter
			...
			$merged_meta  = apply_filters( 'cs_payment_meta', $merged_meta, $payment_data );
			...
			if ( md5( serialize( $this->payment_meta ) ) !== md5( serialize( $merged_meta ) ) ) {
				...
				cs_update_order( $this->ID, $order_info );
		...
		if ( true === $saved ) {
			 ...
			 */
			do_action( 'cs_payment_saved', $this->ID, $this );
		}
		 ...
		 */
		$cache_key = md5( 'cs_payment' . $this->ID );
	...
	public function add_download( $download_id = 0, $args = array(), $options = array() ) {
		...
		$args = wp_parse_args( apply_filters( 'cs_payment_add_download_args', $args, $download->ID ), $defaults );
		...
		if ( false !== $args['item_price'] ) {
			...
			if ( cs_has_variable_prices( $download->ID ) ) {
				$prices = get_post_meta( $download->ID, 'cs_variable_prices', true );
				...
				if ( $args['price_id'] && array_key_exists( $args['price_id'], (array) $prices ) ) {
					...
					$args['price_id'] = cs_get_lowest_price_id( $download->ID );
				}
			} else {
		...
		$quantity   = cs_item_quantities_enabled() ? absint( $args['quantity'] ) : 1;
		$amount     = round( $item_price * $quantity, cs_currency_decimal_filter() );
		...
		if ( cs_prices_include_tax() ) {
			$subtotal -= round( $tax, cs_currency_decimal_filter() );
		}
		...
		if ( ! empty( $args['fees'] ) && is_array( $args['fees'] ) ) {
			...
			$fees = round( $fees, cs_currency_decimal_filter() );
		}
		...
		$this->cart_details[] = array(
			...
			'item_price'  => round( $item_price, cs_currency_decimal_filter() ),
			'quantity'    => $quantity,
			'discount'    => $discount,
			'subtotal'    => round( $subtotal, cs_currency_decimal_filter() ),
			'tax'         => round( $tax, cs_currency_decimal_filter() ),
			'fees'        => $args['fees'],
			'price'       => round( $total, cs_currency_decimal_filter() ),
		);
	...
	public function remove_download( $download_id, $args = array() ) {
		...
		if ( $orig_quantity > $args['quantity'] ) {
			...
			$total_reduced = round( $item_price * $args['quantity'], cs_currency_decimal_filter() );
			$tax_reduced   = round( ( $tax / $orig_quantity ) * $args['quantity'], cs_currency_decimal_filter() );
		...
		/** @var CS\Orders\Order_Item $order_item */
		...
		if ( ! is_null( $order_item ) ) {
			...
			if ( isset( $this->cart_details[ $found_cart_key ] ) ) {
				cs_update_order_item( $order_item->id, array(
				...
				cs_delete_order_item( $order_item->id );
			}
		}
	...
	public function modify_cart_item( $cart_index = false, $args = array() ) {
		...
		$allowed_items = apply_filters( 'cs_allowed_cart_item_modifications', array(
			'item_price',
			'tax',
			'discount',
			'quantity',
		...
		$merged_item['item_price'] = cs_sanitize_amount( $merged_item['item_price'] );
		...
		$merged_item['tax']                 = cs_sanitize_amount( $merged_item['tax'] );
		$merged_item['price']               = cs_prices_include_tax() ? $new_subtotal - $discount : $new_subtotal + $merged_item['tax'] - $discount;
		...
		if ( $order_item_id ) {
			cs_update_order_item( $order_item_id, array(
	...
	public function remove_fee_by( $key, $value, $global = false ) {
		$allowed_fee_keys = apply_filters( 'cs_payment_fee_keys', array(
			'index',
			'label',
			'amount',
			'type',
		...
		if ( true === $removed ) {
			...
			if ( ! is_null( $fee_id ) ) {
				foreach ( $this->order->get_fees() as $id => $f ) {
					if ( $id === $fee_id ) {
						cs_delete_order_adjustment( $f->id );
	...
	public function get_fees( $type = 'all' ) {
		...
		return apply_filters( 'cs_get_payment_fees', $fees, $this->ID, $this );
	}
	...
	public function add_note( $note = '' ) {
		...
		$note_id = cs_insert_payment_note( $this->ID, $note );
	...
	public function update_status( $status = '' ) {
		...
		$do_change = apply_filters( 'cs_should_update_payment_status', true, $this->ID, $status, $old_status );

		$updated = false;

		if ( $do_change ) {
			do_action( 'cs_before_payment_status_change', $this->ID, $status, $old_status );

			$update_fields = apply_filters( 'cs_update_payment_status_fields', array(
				'status' => $status,
			) );

			// Account for someone filtering and using `post_status`
			if ( isset( $update_fields['post_status'] ) ) {
				_cs_generic_deprecated( 'CS_Payment::update_status', '3.0', __( 'Array key "post_status" is no longer a supported attribute for the "cs_update_payment_status_fields" filter. Please use "status" instead.', 'commercestore' ) );

				$update_fields['status'] = $update_fields['post_status'];
				unset( $update_fields['post_status'] );
			}

			// Strip data that does not need to be passed to `cs_update_order()`.
			unset( $update_fields['ID'] );

			/**
			 * As per the new refund API introduced in 3.0, the order is only
			 * marked as refunded when `CS_Payment::process_refund()` has called
			 * `cs_refund_order()` and a new order has been generated with a
			 * type of `refund`.
			 *
			 * @since 3.0
			 * @see CS_Payment::process_refund()
			 * @see cs_refund_order()
			 * @see https://github.com/commercestore/commercestore/issues/2721
			 */
			if ( 'refunded' !== $status ) {
				cs_update_order( $this->ID, $update_fields );

				// Update each order item.
				foreach ( $this->order->items as $item ) {
					cs_update_order_item( $item->id, $update_fields );
				}
			}

			/**
			 * Albeit the order itself is not updated (for refunds), the CS_Payment
			...
			switch ( $status ) {
				case 'refunded':
					$this->process_refund();
					do_action( 'cs_update_payment_status', $this->ID, $status, $old_status );
					break;
				case 'failed':
	...
	public function get_meta( $meta_key = '_cs_payment_meta', $single = true ) {
		...
		if ( '_cs_payment_meta' === $meta_key ) {
			if ( empty( $meta ) ) {
				$meta = array();
			}

			// Payment meta was simplified in CommerceStore v1.5, so these are here for backwards compatibility
			if ( empty( $meta['key'] ) ) {
				$meta['key'] = $this->key;
			}
			...
			$migrated_payment_meta = cs_get_order_meta( $this->ID, 'payment_meta', true );

			// This is no longer stored in _cs_payment_meta.
		...
		$meta = apply_filters( 'cs_get_payment_meta_' . $meta_key, $meta, $this->ID );
		...
		return apply_filters( 'cs_get_payment_meta', $meta, $this->ID, $meta_key );
	}
	...
	public function update_meta( $meta_key = '', $meta_value = '', $prev_value = '' ) {
		...
		$meta_value = apply_filters( 'cs_update_payment_meta_' . $meta_key, $meta_value, $this->ID );
		...
		switch ( $meta_key ) {
			case '_cs_payment_meta':
				...
				if ( isset( $meta_value['user_info'] ) && ! empty( $meta_value['user_info'] ) ) {
					...
					if ( ! empty( $discounts ) && ( 'none' !== $discounts[0] ) ) {
						foreach ( $discounts as $discount ) {
							...
							if ( 1 === count( $found_discount ) ) {
								...
								/** @var CS\Orders\Order_Adjustment $found_discount */

								cs_update_order_adjustment( $found_discount->id, array(
					...
					if ( null !== $this->order && $this->order->get_address()->id ) {
						...
						cs_update_order_address( $order_address->id, array(
							'name'        => $name,
					...
					if ( ! empty( $remaining_user_info ) ) {
						cs_update_order_meta( $this->ID, 'user_info', $remaining_user_info );
					}
				}
				...
				if ( isset( $meta_value['fees'] ) && ! empty( $meta_value['fees'] ) ) {
					foreach ( $meta_value['fees'] as $fee_id => $fee ) {
						if ( ! empty( $fee['download_id'] ) && 0 < $fee['download_id'] ) {
							$order_item_id = cs_get_order_items( array(
								'number'     => 1,
								'order_id'   => $this->ID,
							...
							$adjustment_id = cs_get_order_adjustments( array(
								'number'      => 1,
							...
							if ( is_array( $adjustment_id ) && ! empty( $adjustment_id ) ) {
								...
								cs_update_order_adjustment( $adjustment_id, array(
									'description' => $fee['label'],
				 ...
				 * purposes only. Due to the way CS_Payment, the cart_details array needs to be synchronized with the data
				 * stored in the database as it could be different to the other class vars in the instance of CS_Payment.
				 */
				...
				return cs_update_order_meta( $this->ID, 'payment_meta', $meta_value );
			case '_cs_completed_date':
			...
			case '_cs_payment_tax_rate':
				...
				cs_update_order_meta( $this->ID, 'tax_rate', $tax_rate, $prev_value );
				return true;
			case '_cs_payment_customer_id':
				cs_update_order( $this->ID, array(
			...
			case 'transaction_id':
				...
				if ( $transaction_ids ) {
					...
					return cs_update_order_transaction( $transaction_id, array(
		...
		return cs_update_order_meta( $this->ID, $meta_key, $meta_value, $prev_value );
	}
	...
	public function add_meta( $meta_key = '', $meta_value = '', $unique = false ) {
		...
		return cs_add_order_meta( $this->ID, $meta_key, $meta_value, $unique );
	}
	...
	public function delete_meta( $meta_key = '', $meta_value = '' ) {
		...
		return cs_delete_order_meta( $this->ID, $meta_key, $meta_value );
	}
	...
	private function process_refund() {
		...
		$process_refund = apply_filters( 'cs_should_process_refund', $process_refund, $this );

		if ( false === $process_refund ) {
			return;
		}

		do_action( 'cs_pre_refund_payment', $this );

		$decrease_store_earnings = apply_filters( 'cs_decrease_store_earnings_on_refund', true, $this );
		$decrease_customer_value = apply_filters( 'cs_decrease_customer_value_on_refund', true, $this );
		$decrease_purchase_count = apply_filters( 'cs_decrease_customer_purchase_count_on_refund', true, $this );
		...
		do_action( 'cs_post_refund_payment', $this );
	}
	...
	private function process_failure() {
		...
		foreach ( $discounts as $discount ) {
			cs_decrease_discount_usage( $discount );
		}
	}
	...
	private function process_pending() {
		...
		// Allow extensions to filter for their own payment types, Example: Recurring Payments
		$process_pending = apply_filters( 'cs_should_process_pending', $process_pending, $this );

		if ( false === $process_pending ) {
			return;
		}

		$decrease_store_earnings = apply_filters( 'cs_decrease_store_earnings_on_pending', true, $this );
		$decrease_customer_value = apply_filters( 'cs_decrease_customer_value_on_pending', true, $this );
		$decrease_purchase_count = apply_filters( 'cs_decrease_customer_purchase_count_on_pending', true, $this );

		$this->maybe_alter_stats( $decrease_store_earnings, $decrease_customer_value, $decrease_purchase_count );

		$this->completed_date = false;
		$this->update_meta( '_cs_completed_date', '' );
	...
	private function maybe_alter_stats( $alter_store_earnings, $alter_customer_value, $alter_customer_purchase_count ) {
		if ( cs_undo_purchase( false, $this->ID ) ) {
			$this->status      = 'refunded';
		...
		if ( true === $alter_store_earnings ) {
			cs_decrease_total_earnings( $this->total );
		}
	...
	private function delete_sales_logs() {
		_doing_it_wrong( __FUNCTION__, 'Sales logs are deprecated and are no longer used.', 'CS 3.0' );
	}
	...
	private function setup_total() {
		$amount = $this->get_meta( '_cs_payment_total', true );

		if ( empty( $amount ) && '0.00' !== $amount ) {
			$meta = $this->get_meta( '_cs_payment_meta', true );
	...
	private function setup_currency() {
		...
		return ! empty( $currency )
			? $currency
			: apply_filters( 'cs_payment_currency_default', cs_get_currency(), $this );
	}
	...
	private function setup_fees() {
		...
		if ( $this->order->get_fees() ) {
			...
			foreach ( $this->order->get_fees() as $order_fee ) {
				/**
				 * @var \CS\Orders\Order_Adjustment $order_fee
				 */
				...
				if ( $id != $order_fee->type_key ) {
					 ...
					 *
					 * @link https://github.com/commercestore/commercestore/issues/8412
					 */
					cs_update_order_adjustment( $order_fee->id, array(
						'type_key' => $id
					) );
				}
	...
	private function setup_transaction_id() {
		$transaction_id = $this->get_meta( '_cs_payment_transaction_id', true );

		if ( empty( $transaction_id ) || (int) $transaction_id === (int) $this->ID ) {
			$gateway        = $this->gateway;
			$transaction_id = apply_filters( 'cs_get_payment_transaction_id-' . $gateway, $this->ID );
		}
	...
	private function setup_user_id() {
		$user_id  = $this->get_meta( '_cs_payment_user_id', true );
		$customer = new CS_Customer( $this->customer_id );
		...
		if ( ! empty( $customer->user_id ) && ( empty( $user_id ) || (int) $user_id !== (int) $customer->user_id ) ) {
			...
			$this->update_meta( '_cs_payment_user_id', $user_id );
		}
	...
	private function setup_email() {
		...
		if ( empty( $email ) ) {
			$email = CS()->customers->get_column( 'email', $this->customer_id );
		}
	...
	private function setup_user_info() {
		...
		$old_meta = cs_get_order_meta( $this->ID, 'payment_meta', true );
	 ...
	 *
	 * @since 2.5
	 * @since 3.0 Refactor to use CS\Orders\Order.
	 *
	...
	private function get_cart_details() {
		return apply_filters( 'cs_payment_cart_details', $this->cart_details, $this->ID, $this );
	}
	...
	private function get_completed_date() {
		...
		return apply_filters( 'cs_payment_completed_date', $date, $this->ID, $this );
	}
	...
	private function get_tax() {
		return apply_filters( 'cs_get_payment_tax', $this->tax, $this->ID, $this );
	}
	...
	private function get_subtotal() {
		return apply_filters( 'cs_get_payment_subtotal', $this->subtotal, $this->ID, $this );
	}
	...
	private function get_discounts() {
		return apply_filters( 'cs_payment_discounts', $this->discounts, $this->ID, $this );
	}
	...
	private function get_discounted_amount() {
		return floatval( apply_filters( 'cs_payment_discounted_amount', $this->order->discount, $this ) );
	}
	...
	private function get_currency() {
		return apply_filters( 'cs_payment_currency_code', $this->currency, $this->ID, $this );
	}
	...
	private function get_gateway() {
		return apply_filters( 'cs_payment_gateway', $this->gateway, $this->ID, $this );
	}
	...
	private function get_transaction_id() {
		return apply_filters( 'cs_get_payment_transaction_id', $this->transaction_id, $this->ID, $this );
	}
	...
	private function get_ip() {
		return apply_filters( 'cs_payment_user_ip', $this->ip, $this->ID, $this );
	}
	...
	private function get_customer_id() {
		return apply_filters( 'cs_payment_customer_id', $this->customer_id, $this->ID, $this );
	}
	...
	private function get_user_id() {
		return apply_filters( 'cs_payment_user_id', $this->user_id, $this->ID, $this );
	}
	...
	private function get_email() {
		return apply_filters( 'cs_payment_user_email', $this->email, $this->ID, $this );
	}
	...
	private function get_user_info() {
		return apply_filters( 'cs_payment_meta_user_info', $this->user_info, $this->ID, $this );
	}
	...
	private function get_address() {
		return apply_filters( 'cs_payment_address', $this->address, $this->ID, $this );
	}
	...
	private function get_key() {
		return apply_filters( 'cs_payment_key', $this->key, $this->ID, $this );
	}
	...
	private function get_downloads() {
		return apply_filters( 'cs_payment_meta_downloads', $this->downloads, $this->ID, $this );
	}
	...
	private function get_unlimited() {
		return apply_filters( 'cs_payment_unlimited_downloads', $this->unlimited, $this->ID, $this );
	}
	 ...
	 * @since 2.8.4
	 *
	 * @return CS_Customer The customer object of the existing customer or new customer.
	 */
	private function maybe_create_customer() {
		...
		if ( empty( $customer->id ) ) {
			$customer = new CS_Customer( $this->email );
		}
