...
export function cardActions() {
	// Update.
	forEach( document.querySelectorAll( '.cs-stripe-update-card' ), function( updateButton ) {
		updateButton.addEventListener( 'click', onToggleUpdateForm );
	} );

	forEach( document.querySelectorAll( '.cs-stripe-cancel-update' ), function( cancelButton ) {
	...
	// Delete.
	forEach( document.querySelectorAll( '.cs-stripe-delete-card' ), function( deleteButton ) {
		deleteButton.addEventListener( 'click', onDeletePaymentMethod );
	} );

	// Set Default.
	forEach( document.querySelectorAll( '.cs-stripe-default-card' ), function( setDefaultButton ) {
...
function onUpdatePaymentMethod( e ) {
	...
	paymentMethodAction( 'csx_update_payment_method', e.target.dataset.source, data )
		.fail( function( response ) {
...
function onDeletePaymentMethod( e ) {
	...
	paymentMethodAction( 'csx_delete_payment_method', e.target.dataset.source );
}
...
function onSetDefaultPaymentMethod( e ) {
	...
	paymentMethodAction( 'csx_set_payment_method_default', e.target.dataset.source );
}
