...
export function paymentMethods() {
	...
	const existingPaymentMethods = document.querySelectorAll( '.cs-stripe-existing-card' );
	...
	if ( 0 !== existingPaymentMethods.length ) {
		...
		let currentPaymentMethod = document.querySelector( '.cs-stripe-existing-card:checked' );

		if ( ! currentPaymentMethod ) {
			currentPaymentMethod = document.querySelector( '.cs-stripe-existing-card:first-of-type' );
...
function getBillingFieldsToggle() {
	return document.getElementById( 'cs-stripe-update-billing-address' );
}
...
function toggleBillingFields( isVisible ) {
	const updateAddressWrapperEl = document.querySelector( '.cs-stripe-update-billing-address-wrapper' );
	...
	const billingAddressPreview = document.querySelector( '.cs-stripe-update-billing-address-current' );
...
function onPaymentSourceChange( paymentSource ) {
	...
	const newCardForm = document.querySelector( '.cs-stripe-new-card' );
	const billingAddressToggle = document.querySelector( '.cs-stripe-update-billing-address-wrapper' );
	...
	// @todo don't be lazy.
	$( '.cs-stripe-card-radio-item' ).removeClass( 'selected' );
	$( paymentSource ).closest( '.cs-stripe-card-radio-item' ).addClass( 'selected' );
	...
	if ( isNew ) {
		...
		// Recalculate taxes.
		if ( window.CS_Checkout.recalculate_taxes ) {
			window.CS_Checkout.recalculate_taxes();
		}
		...
		$( document ).ajaxSuccess( function( event, xhr, options ) {
			...
			if (
				options.data.includes( 'action=cs_get_shop_states' ) &&
				...
				if ( stateField ) {
					...
					// Recalculate taxes.
					if ( window.CS_Checkout.recalculate_taxes ) {
						window.CS_Checkout.recalculate_taxes( stateField.value );
					}
				}
			}
		} );
