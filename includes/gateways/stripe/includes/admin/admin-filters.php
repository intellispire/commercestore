...
function csx_payment_status_filters( $views ) {
	...
	$views['preapproval']         = sprintf( '<a href="%s"%s>%s</a>', esc_url( add_query_arg( 'status', 'preapproval', admin_url( 'edit.php?post_type=download&page=cs-payment-history' ) ) ), $current === 'preapproval' ? ' class="current"' : '', __( 'Preapproved', 'csx' ) . $preapproval_count );
	$views['pending_preapproval'] = sprintf( '<a href="%s"%s>%s</a>', esc_url( add_query_arg( 'status', 'preapproval_pending', admin_url( 'edit.php?post_type=download&page=cs-payment-history' ) ) ), $current === 'preapproval_pending' ? ' class="current"' : '', __( 'Preapproval Pending', 'csx' ) . $preapproval_pending_count );
	$views['cancelled']           = sprintf( '<a href="%s"%s>%s</a>', esc_url( add_query_arg( 'status', 'cancelled', admin_url( 'edit.php?post_type=download&page=cs-payment-history' ) ) ), $current === 'cancelled' ? ' class="current"' : '', __( 'Cancelled', 'csx' ) . $cancelled_count );
...
function csx_payments_column_data( $value, $payment_id, $column_name ) {
	if ( $column_name == 'status' ) {
		...
		if ( in_array( $status, array( 'preapproval', 'preapproval_pending' ), true ) ) {
			$actions[] = '<a href="' . esc_url( add_query_arg( $preapproval_args, admin_url( 'edit.php?post_type=download&page=cs-payment-history' ) ) ) . '">' . __( 'Process', 'csx' ) . '</a>';

			if ( 'cancelled' !== $status ) {
				$actions[] = '<span class="delete"><a href="' . esc_url( add_query_arg( $cancel_args, admin_url( 'edit.php?post_type=download&page=cs-payment-history' ) ) ) . '">' . __( 'Cancel', 'csx' ) . '</a></span>';
			}
		}
