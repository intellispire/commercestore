 ...
 *
 * @todo Replace with CS\Utils\Registry when CS 3.x is required.
 *
 * @package CS_Stripe
 * @since   2.6.19
 */
 ...
 * @abstract
 */
abstract class CS_Stripe_Utils_Registry extends ArrayObject {
	 ...
	 *
	 * @throws Exception if the item does not exist.
	 * @throws CS_Stripe_Exceptions_Attribute_Not_Found if the attribute and/or item does not exist.
	 *
	...
	public function get_attribute( $key, $item_id ) {
		...
		if ( ! empty( $item[ $key ] ) ) {
			...
			throw CS_Stripe_Utils_Exceptions_Attribute_Not_Found::from_attr( $key, $item_id );
		}
