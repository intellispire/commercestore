...
class CS_Fees {
	...
	/**
	 * Setup the CommerceStore Fees
	 *
	 * @since 1.5
	 */
	public function __construct() {
		add_filter( 'cs_payment_meta', array( $this, 'record_fees' ), 10, 2 );
	}
	...
	public function add_fee( $args = array() ) {
		...
		if ( ! empty( $args['download_id'] ) ) {
			...
			if ( ! cs_item_in_cart( $args['download_id'], $options ) ) {
				return false;
			}
		}
		...
		// Sanitize the amount
		$args['amount'] = cs_sanitize_amount( $args['amount'] );

		// Force the amount to have the proper number of decimal places.
		$args['amount'] = number_format( (float) $args['amount'], cs_currency_decimal_filter(), '.', '' );
		...
		// Set the fee
		$fees[ $key ] = apply_filters( 'cs_fees_add_fee', $args, $this );

		// Allow 3rd parties to process the fees before storing them in the session
		$fees = apply_filters( 'cs_fees_set_fees', $fees, $this );

		// Update fees
		CS()->session->set( 'cs_cart_fees', $fees );

		do_action( 'cs_post_add_fee', $fees, $key, $args );
	...
	public function remove_fee( $id = '' ) {
		...
		if ( isset( $fees[ $id ] ) ) {
			...
			do_action( 'cs_post_remove_fee', $fees, $id );
		}
	...
	public function get_fees( $type = 'fee', $download_id = 0, $price_id = NULL ) {
		$fees = CS()->session->get( 'cs_cart_fees' );
		...
		if ( ! empty( $fees ) ) {
			...
			foreach ( $fees as $key => $fee ) {
				...
				if ( ! cs_item_in_cart( $fee['download_id'] ) ) {
					unset( $fees[ $key ] );
				}
			}
		}
		...
		return apply_filters( 'cs_fees_get_fees', ! empty( $fees ) ? $fees : array(), $this );
	}
	...
	public function type_total( $type = 'fee' ) {
		...
		if ( $this->has_fees( $type ) ) {
			foreach ( $fees as $fee ) {
				$total += cs_sanitize_amount( $fee['amount'] );
			}
		}

		return cs_sanitize_amount( $total );
	}
