...
/**
 * CS_HTML_Elements Class
 *
 * @since 1.5
 */
class CS_HTML_Elements {
	...
	public function product_dropdown( $args = array() ) {
		...
		if ( ! current_user_can( 'edit_products' ) ) {
			$product_args['post_status'] = apply_filters( 'cs_product_dropdown_status_nopriv', array( 'publish' ) );
		} else {
			$product_args['post_status'] = apply_filters( 'cs_product_dropdown_status', array(
				'publish',
				'draft',
				'private',
				'future',
		...
		$product_args = apply_filters( 'cs_product_dropdown_args', $product_args );
		...
		if ( $products ) {
			foreach ( $products as $product ) {
				$has_variations = cs_has_variable_prices( $product->ID );
				...
				if ( $args['variations'] ) {
					$prices = cs_get_variable_prices( $product->ID );
		...
		if ( is_array( $args['selected'] ) ) {
			foreach ( $args['selected'] as $item ) {
				if ( ! array_key_exists( $item, $options ) ) {
					...
					$parsed_item = cs_parse_product_dropdown_value( $item );

					if ( $parsed_item['price_id'] !== false ) {
						$prices = cs_get_variable_prices( (int) $parsed_item['download_id'] );
						foreach ( $prices as $key => $value ) {
			...
			if ( ! array_key_exists( $args['selected'], $options ) ) {
				$parsed_item = cs_parse_product_dropdown_value( $args['selected'] );
	...
	public function customer_dropdown( $args = array() ) {
		$defaults = array(
			...
			'selected'      => 0,
			'chosen'        => true,
			'placeholder'   => __( 'Choose a Customer', 'commercestore' ),
			'number'        => 30,
			'data'          => array(
				'search-type'        => 'customer',
				'search-placeholder' => __( 'Search Customers', 'commercestore' ),
			),
			'none_selected' => __( 'No customer attached', 'commercestore' ),
			'required'      => false,
		);
		...
		if ( $customers ) {
			...
			$options[0] = __( 'No customers found', 'commercestore' );
		}
		...
		if ( ! empty( $args['selected'] ) ) {
			...
			if ( ! array_key_exists( $args['selected'], $options ) ) {
				$customer = new CS_Customer( $args['selected'] );
		...
		$output = $this->select( array(
			...
			'selected'         => $args['selected'],
			'id'               => $args['id'],
			'class'            => $args['class'] . ' cs-customer-select',
			'options'          => $options,
			'multiple'         => $args['multiple'],
			'placeholder'      => $args['placeholder'],
			'chosen'           => $args['chosen'],
	...
	public function user_dropdown( $args = array() ) {
		$defaults = array(
			...
			'selected'    => 0,
			'chosen'      => true,
			'placeholder' => __( 'Select a User', 'commercestore' ),
			'number'      => 30,
			'data'        => array(
				'search-type'        => 'user',
				'search-placeholder' => __( 'Search Users', 'commercestore' ),
			),
			'required'    => false,
		);
		...
		if ( $users ) {
			...
			$options[0] = __( 'No users found', 'commercestore' );
		}
		...
		$output = $this->select( array(
			...
			'selected'         => $args['selected'],
			'id'               => $args['id'],
			'class'            => $args['class'] . ' cs-user-select',
			'options'          => $options,
			'multiple'         => $args['multiple'],
			'placeholder'      => $args['placeholder'],
			'chosen'           => $args['chosen'],
	 ...
	 */
	public function discount_dropdown( $name = 'cs_discounts', $selected = 0, $status = '' ) {
		$defaults = array(
			'name'            => 'discounts',
			'id'              => 'discounts',
			'class'           => '',
			...
			'selected'        => 0,
			'chosen'          => true,
			'placeholder'     => __( 'Choose a Discount', 'commercestore' ),
			'show_option_all' => __( 'All Discounts', 'commercestore' ),
			'number'          => 30,
			'data'            => array(
				'search-type'        => 'discount',
				'search-placeholder' => __( 'Search Discounts', 'commercestore' ),
			),
			'required'        => false,
		);
		...
		$discounts = cs_get_discounts( $discount_args );
		$options   = array();
		...
		if ( $discounts ) {
			...
			$options[0] = __( 'No discounts found', 'commercestore' );
		}
	...
	public function category_dropdown( $name = 'cs_categories', $selected = 0 ) {
		...
		$output          = $this->select( array(
			...
			'selected'         => $selected,
			'options'          => $options,
			'show_option_all'  => sprintf( _x( 'All %s', 'plural: Example: "All Categories"', 'commercestore' ), $category_labels['name'] ),
			'show_option_none' => false,
		) );
	 ...
	 */
	public function year_dropdown( $name = 'year', $selected = 0, $years_before = 5, $years_after = 0, $id = 'cs_year_select' ) {
		$current    = date( 'Y' );
	 ...
	 */
	public function month_dropdown( $name = 'month', $selected = 0, $id = 'cs_month_select', $return_long_name = false ) {
		$month    = 1;
		$options  = array();
		$selected = empty( $selected ) ? date( 'n' ) : $selected;

		while ( $month <= 12 ) {
			$options[ absint( $month ) ] = cs_month_num_to_name( $month, $return_long_name );
			$month ++;
		}
	...
	public function country_select( $args = array(), $country = '' ) {
		$args = wp_parse_args(
			...
			array(
				'name'             => 'cs_countries',
				'class'            => 'cs_countries_filter',
				'options'          => cs_get_country_list(),
				'chosen'           => true,
				'selected'         => $country,
				'show_option_none' => false,
				'placeholder'      => __( 'Choose a Country', 'commercestore' ),
				'show_option_all'  => __( 'All Countries', 'commercestore' ),
				'data'             => array(
		...
		if ( false === strpos( $args['class'], 'cs_countries_filter' ) ) {
			$args['class'] .= ' cs_countries_filter';
		}
	...
	public function region_select( $args = array(), $country = '', $region = '' ) {
		...
		$args = wp_parse_args(
			...
			array(
				'name'             => 'cs_regions',
				'class'            => 'cs_regions_filter',
				'options'          => cs_get_shop_states( $country ),
				'chosen'           => true,
				'selected'         => $region,
				'show_option_none' => false,
				'placeholder'      => __( 'Choose a Region', 'commercestore' ),
				'show_option_all'  => __( 'All Regions', 'commercestore' ),
				'required'         => false,
			)
		);

		if ( false === strpos( $args['class'], 'cs_regions_filter' ) ) {
			$args['class'] .= ' cs_regions_filter';
		}
	...
	public function select( $args = array() ) {
		...
		$args = wp_parse_args( $args, array(
			...
			'placeholder'      => null,
			'multiple'         => false,
			'show_option_all'  => _x( 'All', 'all dropdown items', 'commercestore' ),
			'show_option_none' => _x( 'None', 'no dropdown items', 'commercestore' ),
			'data'             => array(),
			'readonly'         => false,
			'disabled'         => false,
			'required'         => false,
		...
		$output = '<select' . $disabled . $readonly . $required . ' name="' . esc_attr( $args['name'] ) . '" id="' . esc_attr( str_replace( '-', '_', $args['id'] ) ) . '" class="cs-select ' . $class . '"' . $multiple . ' data-placeholder="' . $placeholder . '"' . $data_elements . '>';
	...
	public function text( $args = array() ) {
		...
		if ( ! empty( $args['data'] ) ) {
			foreach ( $args['data'] as $key => $value ) {
				$data .= 'data-' . cs_sanitize_key( $key ) . '="' . esc_attr( $value ) . '" ';
			}
		}

		$output = '<span id="cs-' . cs_sanitize_key( $args['name'] ) . '-wrap">';
		if ( ! empty( $args['label'] ) ) {
			$output .= '<label class="cs-label" for="' . cs_sanitize_key( $args['id'] ) . '">' . esc_html( $args['label'] ) . '</label>';
		}

		if ( ! empty( $args['desc'] ) ) {
			$output .= '<span class="cs-description">' . esc_html( $args['desc'] ) . '</span>';
		}
	...
	public function date_field( $args = array() ) {
		...
		if ( empty( $args['class'] ) ) {
			$args['class']          = 'cs_datepicker';
			$args['data']['format'] = cs_get_date_picker_format();

		} elseif ( ! strpos( $args['class'], 'cs_datepicker' ) ) {
			$args['class']          .= ' cs_datepicker';
			$args['data']['format'] = cs_get_date_picker_format();
		}
	...
	public function textarea( $args = array() ) {
		...
		if ( ! empty( $args['label'] ) ) {
			$output .= '<label class="cs-label" for="' . cs_sanitize_key( $args['name'] ) . '">' . esc_html( $args['label'] ) . '</label>';
		}

		$output .= '<textarea name="' . esc_attr( $args['name'] ) . '" id="' . cs_sanitize_key( $args['name'] ) . '" class="' . $class . '"' . $disabled . '>' . esc_attr( $args['value'] ) . '</textarea>';
	...
	public function ajax_user_search( $args = array() ) {
		...
		$args = wp_parse_args( $args, array(
			...
			'name'         => 'user_id',
			'value'        => null,
			'placeholder'  => __( 'Enter Username', 'commercestore' ),
			'label'        => null,
			'desc'         => null,
		...
		$args['class'] = 'cs-ajax-user-search ' . sanitize_html_class( $args['class'] );

		// Concatenate output
		$output  = '<span class="cs_user_search_wrap">';
		$output .= $this->text( $args );
		$output .= '<span class="cs_user_search_results hidden"><span></span></span>';
