...
class CS_Download {
	...
	public function __get( $key = '' ) {
		if ( method_exists( $this, "get_{$key}" ) ) {
			...
			return new WP_Error( 'cs-download-invalid-property', sprintf( __( 'Can\'t get property %s', 'commercestore' ), $key ) );
		}
	}
	...
	public function create( $data = array() ) {
		 ...
		 */
		do_action( 'cs_download_pre_create', $args );
		 ...
		 * @param array $args The post object arguments used for creation.
		 */
		do_action( 'cs_download_post_create', $id, $args );
	...
	public function get_price() {
		...
		if ( ! isset( $this->price ) ) {
			$this->price = get_post_meta( $this->ID, 'cs_price', true );

			if ( $this->price ) {
				$this->price = cs_sanitize_amount( $this->price );
			} else {
				$this->price = 0;
			}
		}
		 ...
		 */
		return apply_filters( 'cs_get_download_price', $this->price, $this->ID );
	}
	...
	public function get_prices() {
		...
		if ( true === $this->has_variable_prices() ) {
			if ( empty( $this->prices ) ) {
				$this->prices = get_post_meta( $this->ID, 'cs_variable_prices', true );
			}
		}
		 ...
		 */
		return apply_filters( 'cs_get_variable_prices', $this->prices, $this->ID );
	}
	...
	public function is_single_price_mode() {
		$ret = $this->has_variable_prices() && get_post_meta( $this->ID, '_cs_price_options_mode', true );
		 ...
		 * @param int|string The ID of the download.
		 */
		return (bool) apply_filters( 'cs_single_price_option_mode', $ret, $this->ID );
	}
	...
	public function has_variable_prices() {
		 ...
		 * @param int|string The ID of the download.
		 */
		return (bool) apply_filters( 'cs_has_variable_prices', $ret, $this->ID );
	}
	...
	public function get_files( $variable_price_id = null ) {
		if ( ! isset( $this->files ) ) {
			...
			$download_files = get_post_meta( $this->ID, 'cs_download_files', true );
		...
		return apply_filters( 'cs_download_files', $this->files, $this->ID, $variable_price_id );
	}
	...
	public function get_file_download_limit() {
		...
		if ( ! isset( $this->file_download_limit ) ) {
			$limit  = get_post_meta( $this->ID, '_cs_download_limit', true );
			$global = cs_get_option( 'file_download_limit', 0 );
		...
		return apply_filters( 'cs_file_download_limit', $this->file_download_limit, $this->ID );
	}
	...
	public function get_refund_window() {
		...
		if ( ! isset( $this->refund_window ) ) {
			$window = get_post_meta( $this->ID, '_cs_refund_window', true );
			$global = cs_get_option( 'refund_window', 0 ); // needs to be 0 here
	...
	public function get_refundability() {
		...
		if ( ! isset( $this->refundability ) ) {
			$default    = 'refundable';
			$refundable = get_post_meta( $this->ID, '_cs_refundability', true );
			$global     = cs_get_option( 'refundability', $default );
	...
	public function get_file_price_condition( $file_key = 0 ) {
		...
		return apply_filters( 'cs_get_file_price_condition', $condition, $this->ID, $files );
	}
	...
	public function get_type() {
		if ( ! isset( $this->type ) ) {
			$this->type = get_post_meta( $this->ID, '_cs_product_type', true );
		...
		return apply_filters( 'cs_get_download_type', $this->type, $this->ID );
	}
	...
	public function get_bundled_downloads() {
		...
		if ( ! isset( $this->bundled_downloads ) ) {
			$this->bundled_downloads = (array) get_post_meta( $this->ID, '_cs_bundled_products', true );
		}

		return (array) apply_filters( 'cs_get_bundled_products', array_filter( $this->bundled_downloads ), $this->ID );
	}
	...
	public function get_notes() {
		...
		if ( ! isset( $this->notes ) ) {
			$this->notes = get_post_meta( $this->ID, 'cs_product_notes', true );
		}

		return (string) apply_filters( 'cs_product_notes', $this->notes, $this->ID );
	}
	...
	public function get_sku() {
		...
		if ( ! isset( $this->sku ) ) {
			...
			$this->sku = get_post_meta( $this->ID, 'cs_sku', true );
		...
		return apply_filters( 'cs_get_download_sku', $this->sku, $this->ID );
	}
	...
	public function get_button_behavior() {
		...
		if ( ! isset( $this->button_behavior ) ) {
			...
			$this->button_behavior = get_post_meta( $this->ID, '_cs_button_behavior', true );

			if ( empty( $this->button_behavior ) || ! cs_shop_supports_buy_now() ) {
				$this->button_behavior = 'add_to_cart';
			}
		}

		return apply_filters( 'cs_get_download_button_behavior', $this->button_behavior, $this->ID );
	}
	...
	public function get_sales() {
		...
		if ( ! isset( $this->sales ) ) {
			...
			if ( '' == get_post_meta( $this->ID, '_cs_download_sales', true ) ) {
				add_post_meta( $this->ID, '_cs_download_sales', 0 );
			}

			$this->sales = get_post_meta( $this->ID, '_cs_download_sales', true );
	...
	public function get_earnings() {
		...
		if ( ! isset( $this->earnings ) ) {
			if ( '' == get_post_meta( $this->ID, '_cs_download_earnings', true ) ) {
				add_post_meta( $this->ID, '_cs_download_earnings', 0 );
			}

			$this->earnings = get_post_meta( $this->ID, '_cs_download_earnings', true );
	...
	public function is_free( $price_id = false ) {
		...
		$is_free = false;
		$variable_pricing = cs_has_variable_prices( $this->ID );

		if ( $variable_pricing && ! is_null( $price_id ) && $price_id !== false ) {
			$price = cs_get_price_option_amount( $this->ID, $price_id );

		} elseif ( $variable_pricing && $price_id === false ) {
			$lowest_price  = (float) cs_get_lowest_price_option( $this->ID );
			$highest_price = (float) cs_get_highest_price_option( $this->ID );
			...
			$price = get_post_meta( $this->ID, 'cs_price', true );
		}
		...
		return (bool) apply_filters( 'cs_is_free_download', $is_free, $this->ID, $price_id );
	}
	...
	public function quantities_disabled() {
		$ret = (bool) get_post_meta( $this->ID, '_cs_quantities_disabled', true );
		return apply_filters( 'cs_download_quantity_disabled', $ret, $this->ID );
	}
	 ...
	 *
	 * NOTE: Currently only checks on cs_get_cart_contents() and cs_add_to_cart()
	 *
	 * @since  2.6.4
	...
	public function can_purchase() {
		...
		return (bool) apply_filters( 'cs_can_purchase_download', $can_purchase, $this );
	}
	...
	public function get_bundle_pricing_variations() {
		return get_post_meta( $this->ID, '_cs_bundled_products_conditions' );
	}
}
