...
function cs_recurring_metabox_head( $download_id ) {
	?>
	<th><?php _e( 'Recurring', 'commercestore' ); ?></th>
	<th><?php _e( 'Free Trial', 'commercestore' ); ?></th>
	<th><?php _e( 'Period', 'commercestore' ); ?></th>
	<th><?php echo _x( 'Times', 'Referring to billing period', 'commercestore' ); ?></th>
	<th><?php echo _x( 'Signup Fee', 'Referring to subscription signup fee', 'commercestore' ); ?></th>
	<?php
}
...
function cs_recurring_price_row_hook( $download_id, $price_id, $args ) {
	...
	<div class="cs-custom-price-option-section">
		...
		if ( version_compare( CS_VERSION, '2.10.999', '>' ) ) {
			printf( '<span class="cs-custom-price-option-section-title">%s</span>', esc_html__( 'Recurring Payments Settings', 'commercestore' ) );
		}
		?>
...
function cs_recurring_metabox_recurring( $download_id, $price_id, $args ) {
	...
	<div class="cs-form-group cs-form-row__column cs-recurring-enabled">
		<label for="cs_variable_prices[<?php echo esc_attr( $price_id ); ?>][recurring]" class="cs-form-group__label"><?php esc_html_e( 'Recurring', 'commercestore' ); ?></label>
		<div class="cs-form-group__control">
			<select name="cs_variable_prices[<?php echo esc_attr( $price_id ); ?>][recurring]" id="cs_variable_prices[<?php echo esc_attr( $price_id ); ?>][recurring]" class="cs-form-group__input">
				<option value="no" <?php selected( $recurring, false ); ?>><?php echo esc_attr_e( 'No', 'commercestore' ); ?></option>
				<option value="yes" <?php selected( $recurring, true ); ?>><?php echo esc_attr_e( 'Yes', 'commercestore' ); ?></option>
			</select>
		</div>
	</div>
	<?php
...
function cs_recurring_metabox_free_trial( $download_id, $price_id, $args ) {
	...
	<fieldset class="cs-form-group cs-form-row__column cs-recurring-free-trial">
		<legend class="cs-form-group__label"><?php esc_html_e( 'Free Trial', 'commercestore' ); ?></legend>
		<div class="cs-form-group__control cs-form-group__control--is-inline">
			<div class="eddrecurring-trial-quantity">
				<label for="cs_variable_prices[<?php echo esc_attr( $price_id ); ?>][trial-quantity]" class="screen-reader-text cs-form-group__label"><?php esc_html_e( 'Trial Quantity', 'commercestore' ); ?></label>
			...
			<div class="eddrecurring-trial-period">
				<label for="cs_variable_prices[<?php echo esc_attr( $price_id ); ?>][trial-unit]" class="screen-reader-text cs-form-group__label"><?php esc_html_e( 'Trial Period', 'commercestore' ); ?></label>
...
function cs_recurring_metabox_period( $download_id, $price_id, $args ) {
	...
	<div class="cs-form-group cs-form-row__column cs-recurring-period">
		<label for="cs_variable_prices[<?php echo esc_attr( $price_id ); ?>][period]" class="cs-form-group__label"><?php esc_html_e( 'Period', 'commercestore' ); ?></label>
		<div class="cs-form-group__control">
...
function cs_recurring_metabox_times( $download_id, $price_id, $args ) {
	...
	<div class="cs-form-row__column cs-form-group times cs-recurring-times">
		<label for="cs_variable_prices[<?php echo esc_attr( $price_id ); ?>][times]" class="cs-form-group__label"><?php echo esc_html_x( 'Times', 'Referring to billing period', 'commercestore' ); ?></label>
		<div class="cs-form-group__control">
...
function cs_recurring_metabox_signup_fee( $download_id, $price_id, $args ) {
	...
	<div class="cs-form-group cs-form-row__column signup_fee cs-recurring-fee">
		<label for="cs_variable_prices[<?php echo esc_attr( $price_id ); ?>][signup_fee]" class="cs-form-group__label"><?php echo esc_html_x( 'Signup Fee', 'Referring to subscription signup fee', 'commercestore' ); ?></label>
		<div class="cs-form-group__control">
			<?php
...
function cs_recurring_metabox_single_recurring( $download_id ) {
	...
	<div class="cs-form-group cs-form-row__column">
		<label for="cs_recurring" class="cs-form-group__label"><?php esc_html_e( 'Recurring', 'commercestore' ); ?></label>
		<div class="cs-form-group__control">
			<select name="cs_recurring" id="cs_recurring" class="cs-form-group__input">
				<option value="no" <?php selected( $recurring, false ); ?>><?php esc_attr_e( 'No', 'commercestore' ); ?></option>
				<option value="yes" <?php selected( $recurring, true ); ?>><?php esc_attr_e( 'Yes', 'commercestore' ); ?></option>
			</select>
		</div>
	</div>
	<?php
...
function cs_recurring_metabox_single_period( $download_id ) {
	...
	?>
	<div class="cs-form-group cs-form-row__column">
		<label for="cs_period" class="cs-form-group__label"><?php esc_html_e( 'Period', 'commercestore' ); ?></label>
		<div class="cs-form-group__control">
...
function cs_recurring_metabox_single_times( $download_id ) {
	...
	<div class="cs-form-group cs-form-row__column">
		<label for="cs_times" class="cs-form-group__label"><?php esc_html_e( 'Times', 'commercestore' ); ?></label>
		<div class="cs-form-group__control">
...
function cs_recurring_metabox_single_signup_fee( $download_id ) {
	...
	<div class="cs-form-group cs-form-row__column">
		<label for="cs_signup_fee" class="cs-form-group__label"><?php esc_html_e( 'Signup Fee', 'commercestore' ); ?></label>
		<div class="cs-form-group__control">
			<?php
			if ( 'before' === $currency_position ) {
				?>
...
function cs_recurring_metabox_trial_options( $download_id ) {
	...
	if( cs_get_option( 'recurring_one_time_discounts' ) ) {
		$one_one_discount_help = ' ' . __( '<strong>Additional note</strong>: with free trials, one time discounts are not supported and discount codes for this product will apply to all payments after the trial period.', 'commercestore' );
	}
	...
	<div id="cs_recurring_free_trial_options_wrap" class="cs-form-group"<?php echo $variable_display; ?>>
		...
		<?php if( cs_is_gateway_active( '2checkout' ) || cs_is_gateway_active( '2checkout_onsite' ) ) : ?>
			<p><strong><?php _e( '2Checkout does not support free trial periods. Subscriptions purchased through 2Checkout cannot include free trials.', 'commercestore' ); ?></strong></p>
		<?php endif; ?>
		...
		<p>
			...
			<label for="cs_recurring_free_trial">
				<?php esc_html_e( 'Enable free trial for subscriptions', 'commercestore' ); ?>
				<span alt="f223" class="cs-help-tip dashicons dashicons-editor-help" title="<?php _e( 'Check this box to include a free trial with subscriptions for this product. When signing up for a free trial, the customer\'s payment details will be taken at checkout but the customer will not be charged until the free trial is completed. <strong>Note:</strong> this only applies when purchasing a subscription. If a price option is not set to recurring, this free trial will not be used.', 'commercestore' ); echo $one_one_discount_help; ?>"></span>
			</label>
		</p>
		<fieldset id="cs_recurring_free_trial_options" class="cs-form-group"<?php echo $option_display; ?>>
			<legend class="screen-reader-text"><?php esc_html_e( 'Free Trial Options', 'commercestore' ); ?></legend>
			<div class="cs-form-group__control cs-form-group__control--is-inline">
				<div class="cs-recurring-trial-quantity">
					<label for="cs_recurring_trial_quantity" class="cs-form-group__label screen-reader-text"><?php esc_html_e( 'Trial Quantity', 'commercestore' ); ?></label>
				...
				<div class="cs-recurring-trial-unit">
					<label for="cs_recurring_trial_unit" class="cs-form-group__label screen-reader-text"><?php esc_html_e( 'Trial Period', 'commercestore' ); ?></label>
...
function cs_recurring_metabox_custom_options( $download_id ) {
	...
	<fieldset id="cs_custom_recurring" class="cs_recurring_custom_wrap cs-form-row"<?php echo $display; ?>>
		<legend><?php esc_html_e( 'Recurring Options for Custom Prices', 'commercestore' ); ?></legend>
		<p><?php esc_html_e( 'Select the recurring options for customers that pay with a custom price.', 'commercestore' ); ?></p>
		<div class="cs-form-group cs-form-row__column">
			<label for="cs_custom_recurring" class="cs-form-group__label"><?php esc_html_e( 'Recurring', 'commercestore' ); ?></label>
			<div class="cs-form-group__control">
		...
		<div class="cs-form-group cs-form-row__column">
			<label for="cs_custom_period" class="cs-form-group__label"><?php esc_html_e( 'Period', 'commercestore' ); ?></label>
			<div class="cs-form-group__control">
		...
		<div class="cs-form-group cs-form-row__column times">
			<label for="cs_custom_times" class="cs-form-group__label"><?php esc_html_e( 'Times', 'commercestore' ); ?></label>
			<div class="cs-form-group__control">
		...
		<div class="cs-form-group cs-form-row__column signup_fee">
			<label for="cs_custom_signup_fee" class="cs-form-group__label"><?php esc_html_e( 'Signup Fee', 'commercestore' ); ?></label>
			<div class="cs-form-group__control">
				<?php
				if ( 'before' === $currency_position ) {
					?>
...
?>
		<div id="cs-order-subscriptions" class="postbox">
			<h3 class="hndle">
				<span><?php _e( 'Subscriptions', 'commercestore' ); ?></span>
			</h3>
			<div class="inside">
				...
				<?php foreach( $subs as $sub ) : ?>
					...
					<?php if( $payments ) : ?>
						<p><strong><?php _e( 'Associated Payments', 'commercestore' ); ?>:</strong></p>
...
function cs_recurring_display_parent_payment( $payment_id = 0 ) {
	...
	<div id="cs-order-subscription-payments" class="postbox">
		<h3 class="hndle">
			<span><?php esc_html_e( 'Subscription', 'commercestore' ); ?></span>
		</h3>
		<div class="inside">
			<p>
				<span class="label"><span class="dashicons dashicons-update"></span> <?php esc_html_e( 'Subscription ID', 'commercestore' ); ?>: <?php printf( '<a href="%s">#%d</a>', esc_url( $sub_url ), (int) $sub->id ); ?></span> (<?php echo esc_html( $sub->get_status_label() ); ?>)
			</p>
			<p><?php esc_html_e( 'Parent Payment', 'commercestore' ); ?>: <?php printf( '<a href="%s">%s</a>', esc_url( $parent_url ), esc_html( cs_get_payment_number( $payment->parent_payment ) ) ); ?></p>
		</div><!-- /.inside -->
...
?>
		<div class="cs-subscription-tx-id cs-admin-box-inside">
			<?php foreach( $subs as $sub ) : ?>
				...
				<p>
					<span class="label"><?php _e( 'Subscription TXN ID:', 'commercestore' ); ?></span>&nbsp;
