...
<div class="cs-confirm-details" id="billing_info">
	<h3><?php _e( 'Please confirm your subscription', 'commercestore' ); ?></h3>
	<p><strong><?php echo $cs_checkout_details['FIRSTNAME'] ?> <?php echo $cs_checkout_details['LASTNAME'] ?></strong><br />
	<?php _e( 'PayPal Status:', 'commercestore' ); ?> <?php echo $cs_checkout_details['PAYERSTATUS'] ?><br />
	<?php _e( 'Email:', 'commercestore' ); ?> <?php echo $cs_checkout_details['EMAIL'] ?></p>
</div>
<table id="order_summary" class="cs-table">
	<thead>
		<tr>
			...
			<th><?php _e( 'Recurring Amount', 'commercestore' ); ?></th>
		</tr>
	</thead>
	<tbody>
		...
		<tr>
			...
			<td>
				...
				<?php if( $subscription->bill_times > 1 ) : ?>
					...
					<br/>
					<span class="cs_subscription_bill_times"><?php printf( _n( '%d Time', '%d Times', $subscription->bill_times, 'commercestore' ), $subscription->bill_times ); ?></span>
				<?php endif; ?>
			</td>
		</tr>
		<?php endforeach; ?>
...
<form action="<?php echo esc_url( add_query_arg( 'cs-confirm', 'paypal_express' ) ); ?>" method="post">
	...
	<input type="submit" value="<?php _e( 'Confirm', 'commercestore' ); ?>" />
</form>