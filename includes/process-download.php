...
function cs_process_download() {
	if ( ! isset( $_GET['download_id'] ) && isset( $_GET[CS_EX_DOWNLOAD_ARGS] ) ) {
	...
	if ( ! empty( $args['csfile'] ) && ! empty( $args['ttl'] ) && ! empty( $args['token'] ) ) {
		...
		$_GET['download_id']  = $item[CS_EX_DOWNLOAD_ARGS];
		$_GET['email']        = $args['email'];
		$_GET['expire']       = $args['expire'];
		$_GET['download_key'] = $args['key'];
		$_GET['price_id']     = $args['price_id'];
	} elseif ( ! empty( $item[CS_EX_DOWNLOAD_ARGS] ) && ! empty( $args['key'] ) && ! empty( $args['email'] ) && ! empty( $args['expire'] ) && isset( $args['file_key'] ) ) {
	...
	if ( $args['payment'] && $args['has_access'] ) {
		do_action( 'cs_process_verified_download', $item[CS_EX_DOWNLOAD_ARGS], $args['email'], $args['payment'], $args );
		...
		$download_files = cs_get_download_files( $item[CS_EX_DOWNLOAD_ARGS] );
		...
		cs_record_download_in_log( $item[CS_EX_DOWNLOAD_ARGS], $args['file_key'], $user_info, cs_get_ip(), $args['payment'], $args['price_id'] );
		...
		do_action( 'cs_process_download_headers', $requested_file, $item[CS_EX_DOWNLOAD_ARGS], $args['email'], $args['payment'] );
...
function cs_process_legacy_download_url( $args ) {
	...
	// Verify the payment
	$args['payment'] = cs_verify_download_link( $item[CS_EX_DOWNLOAD_ARGS], $args['key'], $args['email'], $args['expire'], $args['file_key'] );
...
function cs_process_signed_download_url( $args ) {
	...
	$item[CS_EX_DOWNLOAD_ARGS]    = $order_parts[1];
	...
	$args['has_access'] = cs_order_grants_access_to_download_files( array(
		...
		'product_id' => $item[CS_EX_DOWNLOAD_ARGS],
