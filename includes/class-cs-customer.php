 ...
 *
 * @since 2.3
 * @since 3.0 No longer extends CS_DB_Customer.
 *
 * @property int $id
 ...
 */
class CS_Customer extends \CS\Database\Rows\Customer {
	...
	public function __construct( $_id_or_email = false, $by_user_id = false ) {
		...
		if ( is_object( $_id_or_email ) ) {
			...
			$customer = cs_get_customer_by( $field, $_id_or_email );
		}
		...
		switch ( $key ) {
			...
			default:
				return isset( $this->{$key} )
					? $this->{$key}
					: cs_get_customer_meta( $this->id, $key );
		}
	}
	...
	public function create( $data = array() ) {
		 ...
		 */
		do_action( 'cs_customer_pre_create', $args );
		...
		if ( ! empty( $customer_id ) ) {
			...
			cs_add_customer_email_address( array(
		 ...
		 * @param array $args Contains customer information such as payment ID, name, and email.
		 */
		do_action( 'cs_customer_post_create', $created, $args );
	...
	public function update( $data = array() ) {
		...
		do_action( 'cs_customer_pre_update', $this->id, $data );

		$updated          = false;
		$previous_user_id = $this->user_id;

		if ( cs_update_customer( $this->id, $data ) ) {
			$customer = cs_get_customer( $this->id );
			$this->setup_customer( $customer );
			...
			if ( intval( $previous_user_id ) !== intval( $this->user_id ) ) {
				...
				$order_ids = cs_get_orders( array( 'customer_id' => $this->id, 'number' => 9999999 ) );

				foreach ( $order_ids as $order_id ) {
					cs_update_order( $order_id, array( 'user_id' => $this->user_id ) );
				}
		...
		do_action( 'cs_customer_post_update', $updated, $this->id, $data );
	...
	public function add_email( $email = '', $primary = false ) {
		...
		do_action( 'cs_customer_pre_add_email', $email, $this->id, $this );
		...
		$ret = (bool) cs_add_customer_email_address( array(
			'customer_id' => $this->id,
			'email'       => $email,
			'type'        => $type
		) );

		do_action( 'cs_customer_post_add_email', $email, $this->id, $this );
	...
	public function remove_email( $email = '' ) {
		...
		do_action( 'cs_customer_pre_remove_email', $email, $this->id, $this );

		$email_address = cs_get_customer_email_address_by( 'email', $email );

		$ret = $email_address
			? (bool) cs_delete_customer_email_address( $email_address->id )
			: false;

		do_action( 'cs_customer_post_remove_email', $email, $this->id, $this );
	 ...
	 * of WordPress Users, or CommerceStore customer email addresses.
	 *
	 * We intentionally skip the cs_customers table, to avoid race conditions
	...
	public function email_exists( $email = '' ) {
		...
		$exists = cs_get_customer_email_address_by( 'email' , $email );
	...
	public function set_primary_email( $new_primary_email = '' ) {
		...
		do_action( 'cs_customer_pre_set_primary_email', $new_primary_email, $this->id, $this );
		...
		// Get customer emails
		$emails = cs_get_customer_email_addresses( array(
			'customer_id' => $this->id
		) );
		...
		if ( ! in_array( $this->email, $plucked, true ) ) {
			...
			$added = cs_add_customer_email_address( array(
				'customer_id' => $this->id,
				'email'       => $this->email,
				'type'        => 'primary'
			) );
		...
		foreach ( $emails as $email ) {
			...
			if ( ( 'primary' === $email->type ) && ( $new_primary_email !== $email->email ) ) {
				cs_update_customer_email_address( $email->id, array(
					'type' => 'secondary'
				) );
			}
		...
		do_action( 'cs_customer_post_set_primary_email', $new_primary_email, $this->id, $this );
	...
	public function update_order_email_addresses( $email = '' ) {
		...
		foreach ( $payment_ids as $payment_id ) {
			cs_update_payment_meta( $payment_id, 'email', $email );
		}
	}
	...
	public function get_payment_ids() {
		...
		// Get total orders
		$count = cs_count_orders( array(
			'customer_id' => $this->id
		) );

		// Get order IDs
		$ids = cs_get_orders( array(
			'customer_id'   => $this->id,
			'number'        => $count,
			'fields'        => 'ids',
			'no_found_rows' => true
	...
	/**
	 * Get an array of CS_Payment objects from the payment_ids attached to the customer.
	 *
	 * @since 2.6
	 *
	 * @param  array|string  $status A single status as a string or an array of statuses.
	 * @return array An array of CS_Payment objects or an empty array.
	 */
	...
	public function attach_payment( $payment_id = 0, $update_stats = true ) {
		...
		do_action( 'cs_customer_pre_attach_payment', $order->id, $this->id, $this );

		// Update the order
		$success = (bool) cs_update_order( $payment_id, array(
			'customer_id' => $this->id,
			'email'       => $this->email
		) );
		...
		do_action( 'cs_customer_post_attach_payment', $success, $order->id, $this->id, $this );
	...
	public function remove_payment( $payment_id = 0, $update_stats = true ) {
		...
		do_action( 'cs_customer_pre_remove_payment', $payment->ID, $this->id, $this );
		...
		do_action( 'cs_customer_post_remove_payment', $success, $payment->ID, $this->id, $this );
	...
	public function recalculate_stats() {
		$this->purchase_count = cs_count_orders(
			array(
				'customer_id' => $this->id,
				'status'      => cs_get_net_order_statuses(),
				'type'        => 'sale',
			)
		);
	...
	public function add_note( $note = '' ) {
		 ...
		 * @return string
		 */
		$note = apply_filters( 'cs_customer_add_note_string', $note );
		 ...
		 * @since 2.3
		 */
		do_action( 'cs_customer_pre_add_note', $note, $this->id, $this );

		// Sanitize note
		$note = trim( wp_kses( stripslashes( $note ), cs_get_allowed_tags() ) );
		 ...
		 */
		do_action( 'cs_customer_post_add_note', '', $note, $this->id, $this );
	...
	public function get_meta( $key = '', $single = true ) {
		return cs_get_customer_meta( $this->id, $key, $single );
	}
	...
	public function add_meta( $meta_key = '', $meta_value = '', $unique = false ) {
		return cs_add_customer_meta( $this->id, $meta_key, $meta_value, $unique );
	}
	...
	public function update_meta( $meta_key = '', $meta_value = '', $prev_value = '' ) {
		return cs_update_customer_meta( $this->id, $meta_key, $meta_value, $prev_value );
	}
	...
	public function delete_meta( $meta_key = '', $meta_value = '' ) {
		return cs_delete_customer_meta( $this->id, $meta_key, $meta_value );
	}
	 ...
	 *
	 * @return array|\CS\Customers\Customer_Address|null Object if primary address requested, array otherwise. Null if no result for primary address.
	 */
	public function get_address( $is_primary = true ) {
		...
		$address = cs_get_customer_addresses( $args );
		if ( ! $is_primary ) {
			return $address;
		}
	 ...
	 *
	 * @return \CS\Customers\Customer_Address[] Array of addresses.
	 */
	...
	public function increase_purchase_count( $count = 1 ) {
		...
		_cs_deprecated_function( __METHOD__, '3.0', 'CS_Customer::recalculate_stats()' );
		...
		do_action( 'cs_customer_pre_increase_purchase_count', $count, $this->id, $this );

		if ( $this->update( array( 'purchase_count' => $new_total ) ) ) {
			$this->purchase_count = $new_total;
		}

		do_action( 'cs_customer_post_increase_purchase_count', $this->purchase_count, $count, $this->id, $this );
	...
	public function decrease_purchase_count( $count = 1 ) {
		...
		do_action( 'cs_customer_pre_decrease_purchase_count', $count, $this->id, $this );

		if ( $this->update( array( 'purchase_count' => $new_total ) ) ) {
			$this->purchase_count = $new_total;
		}

		do_action( 'cs_customer_post_decrease_purchase_count', $this->purchase_count, $count, $this->id, $this );
	...
	public function increase_value( $value = 0.00 ) {
		...
		$value     = floatval( apply_filters( 'cs_customer_increase_value', $value, $this ) );
		$new_value = floatval( $this->purchase_value ) + $value;

		do_action( 'cs_customer_pre_increase_value', $value, $this->id, $this );

		if ( $this->update( array( 'purchase_value' => $new_value ) ) ) {
			$this->purchase_value = $new_value;
		}

		do_action( 'cs_customer_post_increase_value', $this->purchase_value, $value, $this->id, $this );
	...
	public function decrease_value( $value = 0.00 ) {
		...
		$value     = floatval( apply_filters( 'cs_customer_decrease_value', $value, $this ) );
		$new_value = floatval( $this->purchase_value ) - $value;
		...
		do_action( 'cs_customer_pre_decrease_value', $value, $this->id, $this );

		if ( $this->update( array( 'purchase_value' => $new_value ) ) ) {
			$this->purchase_value = $new_value;
		}

		do_action( 'cs_customer_post_decrease_value', $this->purchase_value, $value, $this->id, $this );
