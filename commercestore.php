...
final class CS_Requirements_Check {
	...
	public function load_textdomain() {
		 ...
		 *
		 * In v2.4.6, our textdomain changed from "edd" to "commercestore".
		 *
		 ...
		 *
		 * - wp-content/languages/plugins/commercestore (introduced with language packs)
		 * - wp-content/languages/cs/ (custom folder we have supported since 1.4)
		 * - wp-content/plugins/commercestore/languages/
		 *
		 * In wp-content/languages/cs/ we must look for:
		 * - "commercestore-{lang}_{country}.mo"
		 *
		 * In wp-content/languages/cs/ we must look for:
		 * - "cs-{lang}_{country}.mo" as that was the old file naming convention
		 *
		 * In wp-content/languages/plugins/commercestore/ we only need to look for:
		 * - "commercestore-{lang}_{country}.mo" as that is the new structure
		 *
		 * In wp-content/plugins/commercestore/languages/, we must look for:
	...
	public function load_old_textdomain( $mofile, $textdomain ) {
		...
		if ( ( 'commercestore' === $textdomain ) && ! file_exists( $mofile ) ) {
			$mofile = dirname( $mofile ) . DIRECTORY_SEPARATOR . str_replace( $textdomain, 'cs', basename( $mofile ) );
		}
