...
var CS_Import = {
  ...
  before_submit: function before_submit(arr, form, options) {
    ...
    form.append('<div class="notice-wrap"><div class="cs-progress"><div></div></div></div>'); //check whether client browser fully supports all File API

    if (window.File && window.FileReader && window.FileList && window.Blob) {// HTML5 File API is supported by browser
    } else {
      var import_form = $('.cs-import-form').find('.cs-progress').parent().parent();
      var notice_wrap = import_form.find('.notice-wrap');
      import_form.find('.button:disabled').attr('disabled', false); //Error for older unsupported browsers that doesn't support HTML5 File API
  ...
  complete: function complete(xhr) {
    ...
    if (response.success) {
      var form = $('.cs-import-form .notice-wrap').parent();
      form.find('.cs-import-file-wrap,.notice-wrap').remove();
      form.find('.cs-import-options').slideDown(); // Show column mapping

      var select = form.find('select.cs-import-csv-column'),
...
/******/ });
//# sourceMappingURL=cs-admin-tools-import.js.map