...
class CS_Recurring_Customer {
	...
	static public function set_as_subscriber( $user_id = 0 ) {
		...
		_cs_deprecated_function( __FUNCTION__, '2.4', 'CS_Recurring_Subscriber', $backtrace );

		$user = new WP_User( $user_id );
		$user->add_role( 'cs_subscriber' );

		do_action( 'cs_recurring_set_as_subscriber', $user_id );
	...
	static public function set_customer_id( $user_id = 0, $recurring_id = '' ) {
		...
		$id = apply_filters( 'cs_recurring_set_customer_id', $recurring_id, $user_id );

		return update_user_meta( $user_id, '_cs_recurring_id', $recurring_id );
	...
	static public function get_customer_id( $user_id = 0 ) {
		...
		return get_user_meta( $user_id, '_cs_recurring_id', true );
	...
	static public function set_customer_payment_id( $user_id = 0, $payment_id = 0 ) {
		...
		do_action( 'cs_recurring_set_customer_payment_id', $user_id, $payment_id );
		update_user_meta( $user_id, '_cs_recurring_user_parent_payment_id', $payment_id );
	}
	...
	static public function get_customer_payment_id( $user_id = 0 ) {
		...
		$payment_id = get_user_meta( $user_id, '_cs_recurring_user_parent_payment_id', true );

		// Now look to see if we have updated data
		$customer = new CS_Recurring_Subscriber( $user_id, true );
	...
	static public function set_customer_status( $user_id = 0, $status = 'active' ) {
		...
		$status = apply_filters( 'cs_recurring_set_customer_status', $status, $user_id );
		$ret    = update_user_meta( $user_id, '_cs_recurring_status', $status );
		...
		do_action( 'cs_recurring_set_user_status', $user_id, $status );
	...
	static public function is_customer_active( $user_id = 0 ) {
		...
		return apply_filters( 'cs_recurring_is_user_active', $active, $user_id, $status );
	...
	static public function set_customer_expiration( $user_id = 0, $expiration = 0 ) {
		...
		$date = apply_filters( 'cs_recurring_set_customer_expiration', $expiration, $user_id );
		$ret  = update_user_meta( $user_id, '_cs_recurring_exp', $date );
		...
		do_action( 'cs_recurring_set_user_expiration', $user_id, $expiration );
	...
	static public function get_customer_expiration( $user_id = 0 ) {
		...
		$date = get_user_meta( $user_id, '_cs_recurring_exp', true );
	...
	static public function calc_user_expiration( $_customer_or_user_id = 0, $payment_id = 0 ) {
		...
		$downloads  = cs_get_payment_meta_downloads( $payment_id );
		...
		return apply_filters( 'cs_recurring_calc_expiration', $expiration, $user_id, $payment_id, $period );
	}
