<?php
/**
 * Update the price ID to the subscription rows.
 *
...
if ( class_exists( 'CS_Batch_Export' ) ) {
	 ...
	 * @since 2.9.3
	 */
	class CS_Recurring_Update_Subscription_Price_IDs extends CS_Batch_Export {
		...
		public function get_percentage_complete() {
			...
			$total = (int) get_option( 'cs_recurring_price_id_update_total_count', 0 );
		...
		public function pre_fetch() {
			...
			$sub_count = $wpdb->get_var( "SELECT count(id) FROM {$wpdb->prefix}cs_subscriptions" );
			update_option( 'cs_recurring_price_id_update_total_count', $sub_count );
		}
		...
		private function get_subscription_ids() {
			...
			$sub_ids = $wpdb->get_col( $wpdb->prepare( "SELECT id FROM {$wpdb->prefix}cs_subscriptions LIMIT %d, %d", $offset, $this->per_step ) );
