...
if ( class_exists( 'EDD_Batch_Export' ) ) {
	...
	class EDD_Recurring_Update_Subscription_Price_IDs extends EDD_Batch_Export {
		...
		public function get_percentage_complete() {
			...
			$total = (int) get_option( 'edd_recurring_price_id_update_total_count', 0 );
		...
		public function process_step() {
			...
			if( $had_data ) {
				...
				$this->done = true;
				delete_option( 'edd_recurring_price_id_update_total_count' );
		...
		public function pre_fetch() {
			...
			$sub_count = $wpdb->get_var( "SELECT count(id) FROM {$wpdb->prefix}edd_subscriptions" );
			update_option( 'edd_recurring_price_id_update_total_count', $sub_count );
		}
		...
		private function get_subscription_ids() {
			...
			$sub_ids = $wpdb->get_col( $wpdb->prepare( "SELECT id FROM {$wpdb->prefix}edd_subscriptions LIMIT %d, %d", $offset, $this->per_step ) );
